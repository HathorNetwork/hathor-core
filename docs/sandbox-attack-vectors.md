...
- [Non-Determinism (C1-C7)](#category-c-non-determinism-attacks) - 7 vectors, 3 blocked, 2 partial, 2 unblocked
- [Computational DoS (D1.1-D1.3)](#subcategory-d1-computational-dos) - 3 vectors, 0 blocked ❌
- [Memory DoS (D2.1-D2.6)](#subcategory-d2-memory-dos) - 6 vectors, 0 blocked ❌
- [Time DoS (D3.1-D3.3)](#subcategory-d3-time-dos) - 3 vectors, 1 partial, 2 unblocked
- [Hash Collision DoS (D4.1)](#subcategory-d4-hash-collision-dos) - 1 vector, 1 partial
- [Import Attacks (E1-E2)](#category-e-import-system-attacks) - 2 vectors, 2 blocked ✅

**Critical gaps:** D1, D2, D3 (DoS attacks) - 11 vectors unprotected
...
**Status**: ❌ NOT BLOCKED - Passes AST validation, creates float at runtime
**Impact**: Breaks determinism (float operations are hardware-dependent)
**Priority**: MUST FIX before unrestricted blueprints
...
**Status**: ❌ NOT BLOCKED - `pow()` is allowed, creates float with negative exponent
...
**Status**: ⚠️ PARTIAL - Needs block-derived PYTHONHASHSEED (not yet implemented)
**Impact**: Different nodes may process set operations in different order
**Priority**: MUST FIX before unrestricted blueprints
...
**Status**: ⚠️ PARTIAL - Needs block-derived PYTHONHASHSEED (not yet implemented)
**Impact**: Dictionary operations may differ across nodes
**Priority**: MUST FIX before unrestricted blueprints
...
**Status**: ❌ NOT BLOCKED - No instruction limits
**Impact**: Can freeze node for extended period
**Priority**: CRITICAL - must fix before unrestricted blueprints
...
**Status**: ❌ NOT BLOCKED - No computational limits
**Impact**: Excessive CPU usage
**Priority**: CRITICAL
...
**Status**: ❌ NOT BLOCKED - No algorithmic complexity limits
**Impact**: Excessive CPU and memory usage
**Priority**: CRITICAL
...
**Status**: ❌ NOT BLOCKED - No per-container size limits
**Impact**: Can exhaust node memory
**Priority**: CRITICAL
...
**Status**: ❌ NOT BLOCKED - No string size limits
...
**Status**: ❌ NOT BLOCKED - No memory growth limits
**Impact**: Exponential memory consumption
**Priority**: CRITICAL
...
**Status**: ❌ NOT BLOCKED - No container size limits
**Impact**: Memory exhaustion
**Priority**: CRITICAL
...
**Status**: ❌ NOT BLOCKED - Bypasses per-list limits (each individual list is small)
**Impact**: Exponential memory growth through nesting
**Priority**: CRITICAL
...
**Status**: ❌ NOT BLOCKED - No bigint size limits
**Impact**: Memory exhaustion
**Priority**: CRITICAL
...
**Status**: ⚠️ PARTIAL - Python has default recursion limit (~1000), but still causes DoS before hitting it
**Impact**: Stack exhaustion, node disruption
**Priority**: Should reduce recursion limit further
...
**Status**: ⚠️ PARTIAL - If PYTHONHASHSEED is fixed to a constant, this attack is possible
**Solution**: Derive PYTHONHASHSEED from block hash (deterministic but unpredictable)
**Impact**: Degrades dictionary/set performance to O(n²), causing DoS
**Priority**: MUST FIX - implement block-derived hash seed
...
| **C. Non-Determinism** | 7 | 3 | 2 | 2 | 🔴 Critical Gap |
| **D1. Computational DoS** | 3 | 0 | 0 | 3 | 🔴 Critical Gap |
| **D2. Memory DoS** | 6 | 0 | 0 | 6 | 🔴 Critical Gap |
| **D3. Time DoS** | 3 | 0 | 1 | 2 | 🔴 Critical Gap |
| **D4. Hash Collision DoS** | 1 | 0 | 1 | 0 | 🔴 Critical Gap |
| **E. Import Attacks** | 2 | 2 | 0 | 0 | ✅ Complete |
| **TOTAL** | **30** | **13** | **4** | **13** | |

**Protection Status**: 43% fully protected (13/30), 13% partially protected (4/30), 43% unprotected (13/30)

**Key Finding**: RCE and code injection are well-protected. **The critical gap is DoS attacks—13 unprotected vectors requiring new solutions.**
