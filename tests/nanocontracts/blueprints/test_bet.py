...
class NCBetBlueprintTestCase(unittest.TestCase):
    ...
    def initialize_contract(self):
        ...
        self.date_last_offer = self.get_current_timestamp() + 3600 * 24

        tx = self._get_any_tx()
        context = Context([], tx, b'', timestamp=self.get_current_timestamp())
        runner.call_public_method('initialize', context, oracle_script, self.token_uid, self.date_last_offer)
        self.assertEqual(storage.get('oracle_script'), oracle_script)
        self.assertEqual(storage.get('token_uid'), self.token_uid)
        self.assertEqual(storage.get('date_last_offer'), self.date_last_offer)
    ...
    def test_make_a_bet_after_date_last_offer(self):
        self.initialize_contract()
        with self.assertRaises(TooLate):
            self._make_a_bet(100, '1x1', timestamp=self.date_last_offer + 1)
