...
from hathor.nanocontracts.types import Address, Amount, ContractId, NCActionType, NCDepositAction
...
class MyBlueprint(Blueprint):
    ...
    def deposit(self, ctx: Context) -> None:
        ...
        assert action.type is NCActionType.DEPOSIT
    ...
    def transfer_to(self, ctx: Context, amount: Amount, contract: ContractId, method: str) -> None:
        ...
        actions = [NCDepositAction(token_uid=HTR_TOKEN_UID, amount=amount)]
...
class AttackerBlueprint(Blueprint):
    ...
    def initialize(self, ctx: Context, target: ContractId, n_calls: int) -> None:
        ...
        actions = [NCDepositAction(token_uid=HTR_TOKEN_UID, amount=self.amount)]
    ...
    def _run_attack(self, method: str):
...
class NCReentrancyTestCase(BlueprintTestCase):
    def setUp(self) -> None:
        ...
        # Address1 deposits 100 HTR
        actions = [NCDepositAction(token_uid=HTR_TOKEN_UID, amount=100)]
        ctx = Context(actions, tx, self.address1, timestamp=0)
        self.runner.call_public_method(self.nc_target_id, 'deposit', ctx)

        # Address2 deposits 100 HTR
        actions = [NCDepositAction(token_uid=HTR_TOKEN_UID, amount=10_000)]
    ...
    def test_basics(self) -> None:
        ...
        # Address1 sends 30 HTR to attacker contract.
        ctx = Context([], tx, self.address1, timestamp=0)
        ...
        # Address1 tries to send 80 HTR but it fails due to insufficient balance.
