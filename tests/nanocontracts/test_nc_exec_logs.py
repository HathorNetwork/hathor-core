...
from hathor.nanocontracts.types import ContractId, NCAction, NCActionType, TokenUid, view
...
class MyBlueprint1(Blueprint):
    ...
    def call_another_public(self, ctx: Context, contract_id: ContractId) -> None:
        ...
        actions = [NCAction(type=NCActionType.DEPOSIT, token_uid=TokenUid(b'\x00'), amount=5)]
        result1 = self.call_public_method(contract_id, 'sum', actions, 1, 2)
        result2 = self.call_view_method(contract_id, 'hello_world')
