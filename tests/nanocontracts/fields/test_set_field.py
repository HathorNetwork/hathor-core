...
class TestDequeField(unittest.TestCase):
    ...
    def test_set_field(self) -> None:
        ...
        for i in range(1, 6):
            assert b11_storage.get(self._get_key(i)) == i

        for i in (0, 6):
            assert not b11_storage.contains(self._get_key(i))
        ...
        for i in range(2, 5):
            assert b12_storage.get(self._get_key(i)) == i

        for i in (1, 5):
            assert not b12_storage.contains(self._get_key(i))
