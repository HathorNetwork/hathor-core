...
from tests.utils import HAS_ROCKSDB, get_genesis_key
...
class BasePoaSimulationTest(SimulatorTestCase):
    ...
    def test_use_case(self) -> None:
        ...
        self.simulator.settings = settings._replace(
            MIN_BLOCK_WEIGHT=0,
            MIN_TX_WEIGHT_K=0,
        ...
        token_tx = TokenCreationTransaction(
            timestamp=settings.GENESIS_BLOCK_TIMESTAMP + 3,
            weight=0,
            parents=[settings.GENESIS_TX1_HASH, settings.GENESIS_TX2_HASH],
            inputs=[TxInput(settings.GENESIS_BLOCK_HASH, 0, b'')],
            outputs=[
                TxOutput(100_000_000_000_00, script, 0b00000001),
