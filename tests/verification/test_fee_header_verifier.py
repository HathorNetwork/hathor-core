...
from hathor.transaction.exceptions import FeeHeaderInvalidAmount, FeeHeaderTokenNotFound, InvalidFeeHeader
...
class TestFeeHeaderVerifier(unittest.TestCase):
    ...
    def test_verify_without_storage_valid_cases(self) -> None:
        ...
        FeeHeaderVerifier.verify_without_storage(header, len(tx.tokens) + 1)  # +1 for HTR
        ...
        FeeHeaderVerifier.verify_without_storage(header, len(tx.tokens) + 1)
    ...
    def test_invalid_token_index_greater_than_tx_tokens_len(self) -> None:
        ...
        tx_tokens_len = len(tx.tokens) + 1  # 2
        fees = [FeeHeaderEntry(token_index=3, amount=100)]  # Index 3 > tx_tokens_len (2)
        header = self._create_fee_header(tx, fees)

        with pytest.raises(FeeHeaderTokenNotFound,
                           match="fees contains token index 3 which is not in tokens list"):
            FeeHeaderVerifier.verify_without_storage(header, tx_tokens_len)

    def test_fee_entry_validation_positive_amount(self) -> None:
        """Test FeeEntry validation for positive amounts."""
        # Valid positive amount
        fee_entry = FeeEntry(token_uid=self._settings.HATHOR_TOKEN_UID, amount=100)
        assert fee_entry.amount == 100

        # Invalid zero amount
        with pytest.raises(FeeHeaderInvalidAmount, match="fees should be a positive integer, got 0"):
            FeeEntry(token_uid=self._settings.HATHOR_TOKEN_UID, amount=0)

        # Invalid negative amount
        with pytest.raises(FeeHeaderInvalidAmount, match="fees should be a positive integer, got -50"):
            FeeEntry(token_uid=self._settings.HATHOR_TOKEN_UID, amount=-50)

    def test_fee_entry_validation_custom_token_multiple_of_100(self) -> None:
        """Test FeeEntry validation for custom tokens requiring multiples of 100."""
        custom_token_uid = TokenUid(b'custom_token_uid_32_bytes_long!')

        # Valid multiple of 100
        fee_entry = FeeEntry(token_uid=custom_token_uid, amount=100)
        assert fee_entry.amount == 100

        fee_entry = FeeEntry(token_uid=custom_token_uid, amount=500)
        assert fee_entry.amount == 500

        # Invalid non-multiple of 100
        with pytest.raises(FeeHeaderInvalidAmount,
                           match="fees using deposit custom tokens should be a multiple of 100, got 150"):
            FeeEntry(token_uid=custom_token_uid, amount=150)

    def test_fee_entry_validation_htr_token_any_amount(self) -> None:
        """Test that HTR token fees can be any positive amount (not restricted to multiples of 100)."""
        # HTR can use any positive amount
        fee_entry = FeeEntry(token_uid=self._settings.HATHOR_TOKEN_UID, amount=123)
        assert fee_entry.amount == 123

        fee_entry = FeeEntry(token_uid=self._settings.HATHOR_TOKEN_UID, amount=1)
        assert fee_entry.amount == 1
