...
class BaseConsensusSimulatorTestCase(SimulatorTestCase):
    ...
    async def do_step(self, i: int, manager1: HathorManager, tx_base: Transaction) -> Transaction:
        ...
        txA = await add_custom_tx(manager1, [(tx_base, 0)], n_outputs=2, address=address)
        self.graphviz.labels[txA.hash] = f'txA-{i}'

        txB = await add_custom_tx(manager1, [(txA, 0)], address=address)
        self.graphviz.labels[txB.hash] = f'txB-{i}'
        txC = await add_custom_tx(manager1, [(txA, 1)], address=address)
        self.graphviz.labels[txC.hash] = f'txC-{i}'

        txD1 = await add_custom_tx(manager1, [(txC, 0)], base_parent=tx_base, address=address)
        self.graphviz.labels[txD1.hash] = f'txD1-{i}'
        txF2 = await add_custom_tx(manager1, [(txB, 0), (txD1, 0)], address=address)
        self.graphviz.labels[txF2.hash] = f'txF2-{i}'

        txD2 = await add_custom_tx(manager1, [(txC, 0)], base_parent=tx_base, inc_timestamp=1, address=address)
        self.graphviz.labels[txD2.hash] = f'txD2-{i}'
        txE = await add_custom_tx(manager1, [(txD2, 0)], base_parent=tx_base, address=address)
        self.graphviz.labels[txE.hash] = f'txE-{i}'

        txF1 = await add_custom_tx(manager1, [(txB, 0)], base_parent=tx_base, address=address)
        ...
        txG = await add_custom_tx(manager1, [(txF2, 0)], base_parent=tx_base, address=address)
        self.graphviz.labels[txG.hash] = f'txG-{i}'

        txH = await add_custom_tx(manager1, [(txF1, 0), (txG, 0)], address=address)
    ...
    async def _get_txF_hashes(self) -> list[VertexId]:
    ...
    async def test_soft_voided(self) -> None:
        txF_hashes = await self._get_txF_hashes()
