...
mod tests {
    ...
    fn test_full_unshield_custom_token_only_without_excess_is_rejected() {
        let gen = derive_asset_tag(&CUSTOM_TOKEN_UID).unwrap();
    ...
    fn test_full_unshield_hybrid_inputs_without_excess_is_rejected() {
        ...
        let vbf_in = Tweak::new(&mut rand::thread_rng());
        let c_shielded_in = create_commitment(500, &vbf_in, &gen).unwrap();
    ...
    fn test_full_unshield_htr_and_custom_token_with_excess() {
        ...
        let excess = compute_balancing_blinding_factor(
            ...
            &ZERO_TWEAK,
            &[
                (500, vbf_htr, ZERO_TWEAK),
                (300, vbf_custom, ZERO_TWEAK),
            ],
            &[
                (500, ZERO_TWEAK, ZERO_TWEAK),
                (300, ZERO_TWEAK, ZERO_TWEAK),
            ],
        )
        .unwrap();
    ...
    fn test_hybrid_inputs_to_shielded_output() {
        ...
        let c_shielded_in = create_commitment(500, &vbf_shielded_in, &gen).unwrap();

        let vbf_out = compute_balancing_blinding_factor(
            1000,
            &ZERO_TWEAK,
            &[(500, ZERO_TWEAK, ZERO_TWEAK), (500, vbf_shielded_in, ZERO_TWEAK)],
            &[],
        )
        .unwrap();
        let c_out = create_commitment(1000, &vbf_out, &gen).unwrap();
    ...
    fn test_hybrid_inputs_to_hybrid_outputs() {
        ...
        .unwrap();
        let c_shielded_out = create_commitment(700, &vbf_out, &gen).unwrap();
