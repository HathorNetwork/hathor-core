...
from hathorlib.checkpoint import Checkpoint
from hathorlib.conf.feature_setting import FeatureSetting
from hathorlib.conf.utils import parse_hex_str
from hathorlib.consensus.consensus_settings import ConsensusSettings, PowSettings
from hathorlib.feature_activation.settings import Settings as FeatureActivationSettings
...
class HathorSettings(BaseModel):
    ...
    MAX_DISTANCE_BETWEEN_BLOCKS: int = 150 * 30  # 150 * AVG_TIME_BETWEEN_BLOCKS
    ...
    WEIGHT_DECAY_ACTIVATE_DISTANCE: int = 120 * 30  # 120 * AVG_TIME_BETWEEN_BLOCKS default
    ...
    CA_FILEPATH: str = os.path.join(os.path.dirname(__file__), '../cert/ca.crt')

    # Filepath of ca key file to sign connection certificates
    CA_KEY_FILEPATH: str = os.path.join(os.path.dirname(__file__), '../cert/ca.key')
    ...
    MAX_INPUTS_OUTPUTS_ADDRESS_HISTORY: int = 6 * 150  # 6 * MAX_TX_ADDRESSES_HISTORY default
