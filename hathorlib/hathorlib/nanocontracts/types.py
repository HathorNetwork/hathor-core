...
class NCFee:
    ...
    def __get_htr_value__(self, settings: HathorSettings, token_amount_version: TokenAmountVersion) -> TokenAmount:
        """
        Get the amount converted to HTR
        """
        amount = TokenAmount.from_version(self.amount, version=token_amount_version)
        if self.token_uid == HATHOR_TOKEN_UID:
            return amount
        else:
