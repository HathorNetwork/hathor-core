...
class TupleNCType(NCType[tuple]):
    ...
    def _serialize(self, serializer: Serializer, value: tuple, /) -> None:
        from hathorlib.serialization.compound_encoding.collection import encode_collection
        from hathorlib.serialization.compound_encoding.tuple import encode_tuple
        if self._varsize:
    ...
    def _deserialize(self, deserializer: Deserializer, /) -> tuple:
        from hathorlib.serialization.compound_encoding.collection import decode_collection
        from hathorlib.serialization.compound_encoding.tuple import decode_tuple
        if self._varsize:
            assert len(self._args) == 1
