...
from hathorlib.nanocontracts.exception import NCInsufficientFunds, NCTokenAlreadyExists
from hathorlib.nanocontracts.nc_types import NCType
from hathorlib.nanocontracts.storage.contract_storage import (
    AttrKey,
    Balance,
    BalanceKey,
    MutableBalance,
...
class NCChangesTracker(NCContractStorage):
    ...
    def __init__(self, nc_id: ContractId, storage: NCContractStorage):
        ...
        self._balance_diff: dict[BalanceKey, TokenBalance] = {}
    ...
    def get_balance_diff(self) -> MappingProxyType[BalanceKey, TokenBalance]:
    ...
    def _get_mutable_balance(self, token_uid: bytes) -> MutableBalance:
        ...
        balance_diff = self._balance_diff.get(internal_key, TokenBalance(0))
    ...
    def validate_balances_are_positive(self) -> None:
        ...
        for balance_key in self._balance_diff.keys():
            ...
            if balance.value < TokenBalance(0):
    ...
    @override
    def add_balance(self, token_uid: bytes, balance: TokenBalance) -> None:
        self.check_if_locked()
        internal_key = BalanceKey(self.nc_id, token_uid)
        old = self._balance_diff.get(internal_key, TokenBalance(0))
        new = old + balance
