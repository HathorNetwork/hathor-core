...
from hathorlib.nanocontracts.runner.index_records import (
...
class Runner:
    ...
    def __init__(
        ...
        *,
        ...
        self._paid_actions_fees: defaultdict[TokenUid, TokenAmount] = defaultdict(TokenAmount.zero)
    ...
    def _unsafe_call_another_contract_public_method(
        ...
        *,
        ...
        for fee in fees:
            ...
            amount = TokenAmount.from_version(fee.amount, version=self.token_amount_version)
            try:
        ...
        for fee in fees:
            assert fee.amount > 0
            fee_amount = TokenAmount.from_version(fee.amount, version=self.token_amount_version)
            self._update_tokens_amount(
                fee=UpdateTokenBalanceRecord(token_uid=fee.token_uid, amount=-fee_amount.to_balance()),
            )
    ...
    def _reset_all_changes(self) -> None:
        ...
        self._paid_actions_fees = defaultdict(TokenAmount.zero)
            ...
            for record in call.index_updates:
                match record:
                    ...
                    case CreateTokenRecord() | UpdateTokenBalanceRecord():
                        calculated_tokens_totals[record.token_uid] += record.amount.to_balance()
        ...
        for action in ctx.__all_actions__:
            match action:
                case NCDepositAction():
                    token_amount = TokenAmount.from_version(action.amount, version=self.token_amount_version)
                    total_diffs[action.token_uid] -= token_amount.to_balance()
    ...
    def _execute_public_method_call(
        ...
        *,
        ...
        if method is None:
            ...
            if forbid_fallback:
                from hathorlib.nanocontracts.exception import NCMethodNotFound
    ...
    def syscall_mint_tokens(
        ...
        *,
        ...
        token_amount = TokenAmount.from_version(amount, version=self.token_amount_version)
        ...
        self._update_tokens_amount(
            operation=UpdateTokenBalanceRecord(token_uid=token_uid, amount=token_amount.to_balance()),
    ...
    def _create_blueprint_instance(self, blueprint_id: BlueprintId, changes_tracker: NCChangesTracker) -> Blueprint:
        ...
        # TODO: Verify that the OCB's decimal version matches the runner's.
        assert self._call_info is not None
    ...
    def _update_tokens_amount(
        ...
        *,
        ...
        for record in (operation, fee):
            if record is None:
                continue
            changes_tracker.add_balance(record.token_uid, record.amount.to_balance())
            self._updated_tokens_totals[record.token_uid] += record.amount.to_balance()
            call_record.index_updates.append(record)
    ...
    def _validate_actions_fees(
        ...
        fee_sum = TokenAmount.zero()

        # sum of the expected fee calculated by this method
        expected_fee = TokenAmount.zero()
        ...
        for token_uid, actions in ctx_actions.items():
            ...
            if token_info.token_version == TokenVersion.FEE:
                ...
                expected_fee += TokenAmount.from_v1(
