...
class TestValidateFeeAmount(unittest.TestCase):
    ...
    def test_valid_htr_fee(self) -> None:
        ...
        validate_fee_amount(settings, b'\x00', TokenAmount.from_v1(1))
        validate_fee_amount(settings, b'\x00', TokenAmount.from_v1(50))

    def test_zero_amount_raises(self) -> None:
        settings = self._get_settings()
        with self.assertRaises(InvalidFeeAmount):
            validate_fee_amount(settings, b'\x00', TokenAmount.from_v1(0))

    def test_custom_token_valid_multiple(self) -> None:
        settings = self._get_settings()
        # Custom token: amount must be multiple of FEE_DIVISOR
        validate_fee_amount(settings, b'\x01', TokenAmount.from_v1(100))
        validate_fee_amount(settings, b'\x01', TokenAmount.from_v1(200))

    def test_custom_token_not_multiple_raises(self) -> None:
        settings = self._get_settings()
        with self.assertRaises(InvalidFeeAmount):
            validate_fee_amount(settings, b'\x01', TokenAmount.from_v1(50))
