...
class TransactionShieldedAccessorsTest(unittest.TestCase):
    """has_shielded_outputs / has_unshield_balance_header / is_shielded."""

    def test_no_headers(self):
        tx = Transaction()
        self.assertFalse(tx.has_shielded_outputs())
        self.assertFalse(tx.has_unshield_balance_header())
        self.assertFalse(tx.is_shielded())
        with self.assertRaises(ValueError):
            tx.get_shielded_outputs_header()
        with self.assertRaises(ValueError):
            tx.get_unshield_balance_header()

    def test_with_shielded_outputs_header(self):
        tx = Transaction()
        out = _amount_output()
        header = ShieldedOutputsHeader(tx=tx, shielded_outputs=[out])
        tx.headers.append(header)
        self.assertTrue(tx.has_shielded_outputs())
        self.assertFalse(tx.has_unshield_balance_header())
        self.assertTrue(tx.is_shielded())
        self.assertIs(tx.get_shielded_outputs_header(), header)

    def test_with_unshield_balance_header(self):
        tx = Transaction()
        header = UnshieldBalanceHeader(tx=tx, excess_blinding_factor=b'\x00' * 32)
        tx.headers.append(header)
        self.assertFalse(tx.has_shielded_outputs())
...
class VertexParserShieldedGatingTest(unittest.TestCase):
    """The four shielded header IDs are admitted only when ENABLE_SHIELDED_TRANSACTIONS != DISABLED."""

    def test_disabled_omits_shielded_headers(self):
        ...
        settings = HathorSettings(
            ...
            ENABLE_SHIELDED_TRANSACTIONS=FeatureSetting.DISABLED,
        )
        ...
        for header_id in (
            ...
            self.assertNotIn(header_id, supported)
        with self.assertRaises(ValueError):
            VertexParser.get_header_parser(b'\x12', settings)

    def test_enabled_admits_shielded_headers(self):
        from hathorlib.conf.settings import FeatureSetting, HathorSettings
        from hathorlib.vertex_parser import VertexParser
        settings = HathorSettings(
            NETWORK_NAME='unittests',
        ...
        supported = VertexParser.get_supported_headers(settings)
        self.assertIs(supported[VertexHeaderId.SHIELDED_OUTPUTS_HEADER], ShieldedOutputsHeader)
        self.assertIs(supported[VertexHeaderId.UNSHIELD_BALANCE_HEADER], UnshieldBalanceHeader)
        self.assertIs(supported[VertexHeaderId.MINT_HEADER], MintHeader)
        self.assertIs(supported[VertexHeaderId.MELT_HEADER], MeltHeader)
        self.assertIs(VertexParser.get_header_parser(b'\x12', settings), ShieldedOutputsHeader)
        self.assertIs(VertexParser.get_header_parser(b'\x15', settings), MeltHeader)
