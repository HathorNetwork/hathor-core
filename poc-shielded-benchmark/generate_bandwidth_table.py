...
def draw_size_summary(ax, payload_bytes_by_grid):
    ...
    ax.text(50, 92,
            ...
            'Per-output ≈ 4.9 KiB (Borromean range proof dominates).',
            ha='center', va='center', fontsize=9, style='italic', color='#555555')
...
def legend_page(pdf, payload_bytes_by_grid):
    ...
    sections = [
        ...
        ('What goes in the payload',
         'Per shielded output: 33 B Pedersen commitment + 33 B blinded asset generator + '
         '~4.8 KiB Borromean range proof + ~70–140 B asset surjection proof. '
         'The range proof DOMINATES — 96–98% of the payload at any N. The '
         'surjection proof grows ~linearly with N up to about log₂(N) used inputs '
         '(libsecp caps the used-input set).'),
        ('Typical vs peak',
        ...
        ('Caveats',
         ...
         'most-common SKU after OS + moderate apps. The reference baseline range proof '
         'was sized with amount=10¹⁸ (≈60-bit) — a tiny amount like 500 would shrink '
         'the proof to ~750 B because libsecp uses min_bits=0 (auto-min-encoding).'),
    ]
    ...
    sections = [
        ...
        ('Payload sizes',
         [
             ...
             'hathor.crypto.shielded.* over the Rust hathor-ct-crypto library.',
             'Range-proof primitive: secp256k1_rangeproof_sign (Borromean ring sigs),',
             'NOT a Bulletproof — older comments in the repo are wrong.',
         ]),
        ('Disclaimer',
         [
