...
(all times are for ALL M outputs in the transaction, not per-output):
  ...
  CREATION (proof generation):
    1. pedersen_create  — Pedersen commitment + Bulletproof range proof for M outputs
    2. surjection_create — Asset surjection proof for M outputs (given N-input domain)
    3. total_create      — Both combined

  VERIFICATION:
    4. pedersen_verify   — Bulletproof range proof verification for M outputs
...
from hathor.crypto.shielded.range_proof import create_range_proof, verify_range_proof
