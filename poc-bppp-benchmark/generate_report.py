...
def main():
    with PdfPages(PDF_PATH) as pdf:
        ...
        fig.text(0.5, 0.40, 'bppp value commitments, Bulletproofs++ range proofs,',
        ...
        text_page(pdf, '1. Overview', [
            ...
            'shielded transaction: bppp value commitments, Bulletproofs++ (bppp) range '
            'proofs from the distributed-lab/bp-pp Rust crate, and asset surjection '
            'proofs from secp256k1-zkp. This is the bppp counterpart of the report in '
            'poc-shielded-benchmark/, which measures the Borromean variant of the same '
            'pipeline.',
        ...
        text_page(pdf, '2. Methodology', [
            ...
            '(this part is unchanged from the Borromean variant). The bppp value '
            'commitment x*g + s*h_vec[0] then hides the amount and feeds the bppp '
            'range proof. The asset surjection proof still operates on the blinded '
            'asset generators, independent of the value-commitment scheme.',
        ...
        text_page(pdf, '3. Surjection Proofs (Isolated)', [
            ...
            'commitments. The bppp value commitment and range proof are not included here.',
        ], level=2)
        ...
        section_for(pdf, '3.1', 'Surjection: Creation Time',
                    ...
                    [
                        ...
                        'The N dependence is sublinear: the surjection proof internally samples a '
                        'small subset of the domain (Borromean ring construction inside '
                        'secp256k1-zkp), so doubling N raises per-output cost by less than 2x. '
                        'This section is unaffected by the bppp swap — surjection still comes '
                        'from secp256k1-zkp.',
                    ])
        ...
        section_for(pdf, '3.2', 'Surjection: Verification Time',
                    ...
                    [
                        ...
                        'expected asymmetry between proving and verification.',
        ...
        text_page(pdf, '4. Full Per-Output Workload', [
            ...
            'in three pairs (creation, then verification): bppp value commitment + bppp '
            'range proof, asset surjection, and the total. The range proof is fixed-cost '
            'per output (it does not depend on N), so it dominates at small input sets and '
            'gradually loses relative weight as N grows.',

            'Note: verification of the commitment sum (checking that inputs minus outputs '
            'commit to zero) is not included in the figures below. With bppp the commitment '
            'shape is x*g + s*h_vec[0], so an inputs-minus-outputs sum on the value '
            'commitments still produces a single elliptic-curve subgroup check — '
            'sub-millisecond per transaction, negligible next to the per-output range and '
            'surjection proofs.',
        ], level=2)

        section_for(pdf, '4.1', 'bppp Commit + Range Proof: Creation Time',
                    os.path.join(HERE, 'results_full/pedersen_create.csv'),
                    [
                        'Creation cost is essentially flat in N (the bppp commitment and '
                        'range proof do not consult the input set) and grows linearly in M. '
                        'The bppp range proof is the per-output dominant cost; preliminary numbers '
                        'on this hardware are on the order of 30 ms to construct per output, '
                        'roughly 25x more expensive than the Borromean alternative measured in '
                        'poc-shielded-benchmark/ — the trade-off bppp makes for ~9x smaller wire '
                        'bytes (~541 B vs ~5 KB).',
                    ])
        ...
        section_for(pdf, '4.2', 'Range Proof: Verification Time',
                    ...
                    [
                        'Verification is also flat in N and linear in M. Per-output bppp verify '
                        'is in the ~8 ms ballpark on this hardware — substantially cheaper than '
                        'creation, the usual prover/verifier asymmetry for Bulletproofs++-style '
                        'protocols.',
                    ])
        ...
        section_for(pdf, '4.5', 'Total Per-Output Workload: Creation Time',
                    ...
                    [
                        'Sum of bppp commit + range proof + surjection. The bppp range proof '
                        'dominates per-output cost throughout the grid; surjection grows '
                        'sub-linearly in N but stays well below the range proof slot. '
                        'Re-run benchmark_full.py to populate this section\'s numbers.',
                    ])
        ...
        text_page(pdf, '6. Conclusions', [
            ...
            'Per-output cost is dominated by the bppp range proof at any N within the grid — '
            'the bppp prove time is ~30 ms per output, far above surjection. Compared to the '
            'Borromean variant (poc-shielded-benchmark/), bppp trades a ~25x increase in '
            'prover time for ~9x smaller wire bytes (541 B vs ~5000 B per range proof) and '
            'a comparable verification cost. The ZERO_TWEAK trick for transparent inputs in '
            'the surjection domain remains negligibly cheap.',
