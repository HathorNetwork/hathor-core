"""
Comprehensive benchmark: Pedersen + Range Proof + Asset Surjection.
...
(all times are for ALL M outputs in the transaction, not per-output):
  ...
  CREATION (proof generation):
    1. pedersen_create  — bppp value commitment + bppp range proof for M outputs
    2. surjection_create — Asset surjection proof for M outputs (given N-input domain)
    3. total_create      — Both combined

  VERIFICATION:
    4. pedersen_verify   — bppp range proof verification for M outputs
    5. surjection_verify — Asset surjection proof verification for M outputs
    6. total_verify      — Both combined

Range proof is **Bulletproofs++ (bppp)** via the `hathor_bppp` binding (bp-pp crate),
NOT the Borromean range proof from secp256k1-zkp (which is what
`poc-shielded-benchmark/` measures). The metric name `pedersen_create` is kept for
diff-friendliness against the original folder — under the hood the commitment is now
bppp's `commit_value`, not a libsecp Pedersen commitment.

Each (N, M) is run RUNS times; the average is stored.
All outputs are FullShielded (amount + token hidden).
"""
...
from bppp_range_proof import create_commitment, create_range_proof, verify_range_proof  # bppp swap
...
def run_benchmark(max_n: int = MAX_N, max_m: int = MAX_M, runs: int = RUNS):
    ...
    for n_inputs in n_values:
        ...
        for m_outputs in m_values:
            ...
            # Accumulate per-run timings
            timings: dict[str, list[float]] = {m: [] for m in METRICS}

            for run_idx in range(runs):
                # ----- Fresh output secrets each run -----
                out_params = []
                ...
                # --- Pedersen commitment + range proof creation ---
    ...
    for n_inputs in n_values:
        ...
        for m_outputs in m_values:
            ...
            for run_idx in range(runs):
                ...
                # --- Pedersen commitment + range proof creation (per-call) ---
                ...
                # --- Surjection proof creation (per-call) ---
                ...
                # --- Range proof verification (per-call) ---
                ...
                # --- Surjection proof verification (per-call) ---
...
if __name__ == '__main__':
    import argparse
    p = argparse.ArgumentParser(description='Full shielded output benchmark (Pedersen + Surjection)')
