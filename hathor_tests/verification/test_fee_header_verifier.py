...
class TestFeeHeaderVerifier(unittest.TestCase):
    ...
    def test_verify_without_storage_valid_cases(self) -> None:
        ...
        fees = [FeeHeaderEntry(token_index=0, amount=TokenAmount.from_v1(100))]  # HTR fee
        ...
        fees = [
            FeeHeaderEntry(token_index=0, amount=TokenAmount.from_v1(3)),  # HTR
            FeeHeaderEntry(token_index=1, amount=TokenAmount.from_v1(200)),  # Custom token 1
        ]
        header = self._create_fee_header(tx, fees)
    ...
    def test_verify_without_storage_invalid_fee_amounts(self) -> None:
        ...
        with pytest.raises(InvalidFeeAmount, match="fees should be a positive integer, got V2 { normalized: 0 }"):
            fees = [FeeHeaderEntry(token_index=0, amount=TokenAmount.zero())]  # HTR fee
            header = self._create_fee_header(tx, fees)
            FeeHeaderVerifier.verify_fee_list(header, tx)
        ...
        with pytest.raises(
            ...
            fees = [FeeHeaderEntry(token_index=1, amount=TokenAmount.from_v1(150))]
            header = self._create_fee_header(tx, fees)
            FeeHeaderVerifier.verify_fee_list(header, tx)
    ...
    def test_verify_fee_list_size_exceeds_max(self) -> None:
        ...
        fees = [FeeHeaderEntry(token_index=i, amount=TokenAmount.from_v1(100)) for i in range(MAX_FEES_LEN + 1)]
        header = self._create_fee_header(tx, fees)
    ...
    def test_verify_fee_list_size_at_max(self) -> None:
        ...
        fees = [FeeHeaderEntry(token_index=i, amount=TokenAmount.from_v1(100)) for i in range(MAX_FEES_LEN)]
        header = self._create_fee_header(tx, fees)
    ...
    def test_duplicate_token_indexes_in_fees(self) -> None:
        ...
        fees = [
            ...
            FeeHeaderEntry(token_index=0, amount=TokenAmount.from_v1(200)),  # Duplicate HTR fee
        ]
        header = self._create_fee_header(tx, fees)
    ...
    def test_invalid_token_indexes_out_of_bounds(self) -> None:
        ...
        fees = [FeeHeaderEntry(token_index=5, amount=TokenAmount.from_v1(100))]  # Index 5 doesn't exist
        header = self._create_fee_header(tx, fees)
    ...
    def test_invalid_token_index_greater_than_tx_tokens_len(self) -> None:
        ...
        fees = [FeeHeaderEntry(token_index=2, amount=TokenAmount.from_v1(100))]  # Index 2 > tx_tokens_len (1)
        header = self._create_fee_header(tx, fees)
