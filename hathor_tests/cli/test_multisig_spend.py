...
class MultiSigSpendTest(unittest.TestCase):
    ...
    def test_spend_multisig(self):
        ...
        available_tokens = sum(blocks_tokens, start=TokenAmount.zero())
        self.assertEqual(
            self.manager.wallet.balance[self._settings.HATHOR_TOKEN_UID],
            WalletBalance(TokenAmount.zero(), available_tokens)
        )
        ...
        wallet_balance = WalletBalance(TokenAmount.zero(), available_tokens - block_reward)
        ...
        multisig_output = TxOutput(TokenAmount.from_v1(200), multisig_script)
        wallet_output = TxOutput(TokenAmount.from_v1(300), create_output_script(self.address))
        # The remaining change is encoded in V1, matching its sibling outputs and the tx encoding.
        change = (block_reward - TokenAmount.from_v1(200) - TokenAmount.from_v1(300)).to_v1()
