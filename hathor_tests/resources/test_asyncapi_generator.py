...
from hathor.api.asyncapi.generator import AsyncAPIGenerator, ChannelDefinition, MessageDefinition, MessageDirection
...
class TestSchemaRegistryMixin(unittest.TestCase):
    ...
    def test_flatten_schemas_conflicting_defs_raises(self) -> None:
        """Different schemas with the same $def name should raise ValueError."""
        # Manually register two schemas that have conflicting $defs
        self.gen._schemas['ModelA'] = {
            'type': 'object',
            'properties': {'items': {'$ref': '#/components/schemas/Inner'}},
            '$defs': {'Inner': {'type': 'object', 'properties': {'name': {'type': 'string'}}}},
        }
        self.gen._schemas['ModelB'] = {
            'type': 'object',
            'properties': {'items': {'$ref': '#/components/schemas/Inner'}},
            '$defs': {'Inner': {'type': 'object', 'properties': {'value': {'type': 'integer'}}}},
        }

        with self.assertRaises(ValueError) as ctx:
            self.gen._flatten_schemas()
        self.assertIn('Inner', str(ctx.exception))
        self.assertIn('Conflicting', str(ctx.exception))

    def test_flatten_schemas_identical_defs_ok(self) -> None:
        """Identical schemas with the same $def name should not raise."""
        inner_def = {'type': 'object', 'properties': {'name': {'type': 'string'}}}
        self.gen._schemas['ModelA'] = {
            'type': 'object',
            'properties': {'items': {'$ref': '#/components/schemas/Inner'}},
            '$defs': {'Inner': inner_def.copy()},
        }
        self.gen._schemas['ModelB'] = {
            'type': 'object',
            'properties': {'items': {'$ref': '#/components/schemas/Inner'}},
        ...
        flat = self.gen._flatten_schemas()
        self.assertIn('Inner', flat)
        self.assertEqual(flat['Inner'], inner_def)
