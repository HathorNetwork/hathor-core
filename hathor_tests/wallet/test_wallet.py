...
BLOCK_REWARD = TokenAmount.from_v1(300)
...
class BasicWalletTest(unittest.TestCase):
    ...
    def test_wallet_create_transaction(self):
        ...
        genesis_value = sum(([output.value for output in genesis_block.outputs]), start=TokenAmount.zero())
        ...
        self.assertEqual(w.balance[self._settings.HATHOR_TOKEN_UID], WalletBalance(TokenAmount.zero(), genesis_value))

        # create transaction spending this value, but sending to same wallet
        add_blocks_unlock_reward(self.manager)
        new_address = w.get_unused_address()
        out = WalletOutputInfo(decode_address(new_address), TokenAmount.from_v1(100), timelock=None)
        ...
        out = WalletOutputInfo(decode_address(key2.address), TokenAmount.from_v1(100), timelock=None)
    ...
    def test_block_increase_balance(self):
        ...
        self.assertIsNotNone(utxo)
        self.assertEqual(w.balance[self._settings.HATHOR_TOKEN_UID], WalletBalance(TokenAmount.zero(), BLOCK_REWARD))
    ...
    def test_create_token_transaction(self):
        ...
        # token tx
        token_out = WalletOutputInfo(address, tokens_created - TokenAmount.from_v1(20), None, token_uid.hex())
        ...
        token_dict = defaultdict(TokenAmount.zero)
    ...
    def test_prepare_transaction(self):
        ...
        new_address = w.get_unused_address()
        out = WalletOutputInfo(decode_address(new_address), TokenAmount.from_v1(1), timelock=None)
        with self.assertRaises(InsufficientFunds):
