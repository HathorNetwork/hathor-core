...
class WalletHDTest(unittest.TestCase):
    ...
    def test_transaction_and_balance(self):
        ...
        self.assertEqual(
            ...
            WalletBalance(TokenAmount.zero(),self.BLOCK_TOKENS),
        )
        ...
        verifier.verify_script(tx=tx1, input_tx=tx1.inputs[0], spent_tx=block, opcodes_version=OpcodesVersion.V2)
        ...
        self.assertEqual(
            self.wallet.balance[self._settings.HATHOR_TOKEN_UID],
            WalletBalance(TokenAmount.zero(),self.TOKENS),
        )
        ...
        tx2.update_hash()
        tx2.storage = self.tx_storage
        verifier.verify_script(tx=tx2, input_tx=tx2.inputs[0], spent_tx=tx1, opcodes_version=OpcodesVersion.V2)
