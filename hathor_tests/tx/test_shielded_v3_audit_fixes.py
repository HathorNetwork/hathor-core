...
All callers of default_for_mempool must pass explicit features.
...
class TestMempoolCallerFeatures:
    ...
    def test_create_tx_passes_features(self) -> None:
        ...
        assert _method_calls_from_vertex(method), \
            'Regression: CreateTxResource._verify_unsigned_skip_pow must call Features.from_vertex'
        assert _method_passes_features_to_default_for_mempool(method), \
            'Regression: CreateTxResource._verify_unsigned_skip_pow must pass features= to default_for_mempool'
    ...
    def test_wallet_send_tokens_passes_features(self) -> None:
        ...
        assert _method_calls_from_vertex(method), \
            'Regression: SendTokensResource._render_POST_thread must call Features.from_vertex'
        assert _method_passes_features_to_default_for_mempool(method), \
            'Regression: SendTokensResource._render_POST_thread must pass features= to default_for_mempool'
    ...
    def test_thin_wallet_stratum_verify_passes_features(self) -> None:
        ...
        assert _method_calls_from_vertex(method), \
            'Regression: thin_wallet._stratum_thread_verify must call Features.from_vertex'
        assert _method_passes_features_to_default_for_mempool(method), \
            'Regression: thin_wallet._stratum_thread_verify must pass features= to default_for_mempool'

    def test_thin_wallet_render_post_passes_features(self) -> None:
        from hathor.wallet.resources.thin_wallet.send_tokens import SendTokensResource
        method = SendTokensResource._render_POST_thread
        assert _method_calls_from_vertex(method), \
            'Regression: thin_wallet._render_POST_thread must call Features.from_vertex'
        assert _method_passes_features_to_default_for_mempool(method), \
            'Regression: thin_wallet._render_POST_thread must pass features= to default_for_mempool'
    ...
    def test_consensus_opcodes_v2_rule_passes_features(self) -> None:
        ...
        assert _method_calls_from_vertex(method), \
            'Regression: ConsensusAlgorithm._opcodes_v2_activation_rule must call Features.from_vertex'
        assert _method_passes_features_to_default_for_mempool(method), \
            'Regression: _opcodes_v2_activation_rule must pass features= to default_for_mempool'
