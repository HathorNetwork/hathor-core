...
- Feature gate without assert
- Invalid shielded input references
- MAX_SHIELDED_OUTPUTS enforcement
- MAX proof size enforcement in deserialization
- Authority outputs in balance equation
- Out-of-bounds token_data index
- Empty surjection domain
- Token UID length validation
- Header deserialization type check
"""
...
from hathor.transaction.vertex_parser._shielded_outputs_header import deserialize_shielded_outputs_header
...
class TestMaxShieldedOutputs:
    """Enforce MAX_SHIELDED_OUTPUTS limit."""
...
class TestMaxProofSizes:
    """Reject oversized proofs during deserialization."""
...
class TestAuthorityOutputsBalance:
    """Authority outputs should not corrupt balance equation."""
...
class TestTokenDataBoundsCheck:
    """Out-of-bounds token_data index should raise, not crash."""
...
class TestInvalidShieldedInputReferences:
    """Invalid shielded input references should raise, not silently skip."""
...
class TestEmptySurjectionDomain:
    """Empty surjection proof domain should be rejected."""
...
class TestTokenUidValidation:
    """Token UID normalization should reject invalid lengths."""

    def test_valid_1_byte_uid(self) -> None:
        from hathor.verification.transaction_verifier import TransactionVerifier
        _normalize_token_uid = TransactionVerifier._normalize_token_uid
...
class TestHeaderDeserializationTypeCheck:
    """Header deserialization should reject non-Transaction types."""
    ...
    def test_non_transaction_rejected(self) -> None:
        ...
        # The hathorlib header class guards its `tx` argument with a ValueError.
    ...
    def test_malformed_header_caught(self) -> None:
        ...
        with pytest.raises(InvalidShieldedOutputError, match='malformed'):
            deserialize_shielded_outputs_header(Deserializer.build_bytes_deserializer(buf))


class TestHeaderNumOutputsLimits:
    """num_outputs=0 and num_outputs > MAX should be rejected at deserialization."""
