...
class TestSerialization:
    def test_amount_shielded_roundtrip(self) -> None:
        ...
        data = _ser(output)
        restored, remaining = _deser(data)
        assert remaining == b''
    ...
    def test_multiple_outputs_concatenated(self) -> None:
        ...
        data = _ser(o1) + _ser(o2)
        r1, remaining = _deser(data)
        r2, remaining = _deser(remaining)
        assert remaining == b''
...
class TestEphemeralPubkeySerialization:
    ...
    def test_backward_compat_no_ephemeral_pubkey(self) -> None:
        ...
        output = _make_amount_shielded_output()
        assert output.ephemeral_pubkey is None
        data = _ser(output)
        restored, remaining = _deser(data)
        assert remaining == b''
        assert restored.ephemeral_pubkey is None
