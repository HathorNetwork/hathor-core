...
class TimelockTransactionTestCase(unittest.TestCase):
    ...
    def test_timelock(self):
        ...
        blocks_tokens = [sum((txout.value for txout in blk.outputs), start=TokenAmount.zero()) for blk in blocks]
        add_blocks_unlock_reward(self.manager)

        address = self.manager.wallet.get_unused_address()
        outside_address = self.get_address(0)

        value_500 = TokenAmount.from_v1(500)
        value_700 = TokenAmount.from_v1(700)
        outputs = [
            WalletOutputInfo(
            ...
            WalletOutputInfo(
                ...
                value=sum(blocks_tokens[:2], start=TokenAmount.zero()) - value_500 - value_700,
                timelock=None)
        ]
        ...
        all_blocks_sum = sum(blocks_tokens, start=TokenAmount.zero())
        first_three_blocks_sum = sum(blocks_tokens[:3], start=TokenAmount.zero())
        ...
        self.clock.advance(1)
        self.assertEqual(self.manager.wallet.balance[self._settings.HATHOR_TOKEN_UID],
                         WalletBalance(TokenAmount.zero(), first_three_blocks_sum))
        self.assertTrue(propagated)
    ...
    def test_choose_inputs(self):
        ...
        self.assertEqual(self.manager.wallet.balance[self._settings.HATHOR_TOKEN_UID],
                         WalletBalance(blocks_tokens[0], TokenAmount.zero()))
        ...
        self.assertEqual(self.manager.wallet.balance[self._settings.HATHOR_TOKEN_UID],
                         WalletBalance(TokenAmount.zero(), blocks_tokens[0]))
