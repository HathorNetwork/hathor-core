...
# ============================================================================
# IN-001: Startup validation wiring
# ============================================================================

class TestStartupValidation:
    def test_validate_shielded_crypto_available_disabled_ok(self) -> None:
        """With DISABLED setting, validation passes even if crypto lib is unavailable."""
        from unittest.mock import patch
    ...
    def test_validate_shielded_crypto_available_enabled_no_lib_raises(self) -> None:
        """With ENABLED setting, validation raises if crypto lib is unavailable."""
        from unittest.mock import patch
        ...
        with patch('hathor.crypto.shielded.SHIELDED_CRYPTO_AVAILABLE', False):
            with pytest.raises(RuntimeError, match='hathor_ct_crypto native library is not available'):
                validate_shielded_crypto_available(FeatureSetting.ENABLED)

    def test_validate_shielded_crypto_available_feature_activation_no_lib_raises(self) -> None:
        """With FEATURE_ACTIVATION setting, validation raises if crypto lib is unavailable."""
        from unittest.mock import patch
...
class TestEphemeralPubkeyHelpers:
    def test_write_ephemeral_pubkey_absent(self) -> None:
        ...
        from hathor.transaction.shielded_tx_output import EPHEMERAL_PUBKEY_SIZE
    ...
    def test_read_ephemeral_pubkey_absent(self) -> None:
        ...
        output = _make_amount_shielded(ephemeral_pubkey=b'')
        data = serialize_shielded_output(output)
        result, _ = deserialize_shielded_output(data)
        assert result.ephemeral_pubkey == b''
...
class TestInputTypeDiscriminator:
    def test_transparent_inputs_have_type_field(self) -> None:
        ...
        assert "'type': 'shielded'" in source
