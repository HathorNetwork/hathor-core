...
"""Regression tests for shielded outputs audit findings.
...
def _ser(output: ShieldedOutput) -> bytes:
    """Serialize a single shielded output to bytes (test helper over the serializer API)."""
    serializer = Serializer.build_bytes_serializer()
    serialize_shielded_output(serializer, output)
    return bytes(serializer.finalize())


def _deser(data: bytes) -> tuple[ShieldedOutput, bytes]:
    """Deserialize one shielded output from bytes; return (output, leftover)."""
    deserializer = Deserializer.build_bytes_deserializer(data)
    output = deserialize_shielded_output(deserializer)
    return output, bytes(deserializer.read_all())
...
# ============================================================================
# Script length cap in deserialization
# ============================================================================

class TestScriptLengthCap:
    ...
    def test_shielded_output_accepts_max_script(self) -> None:
        ...
        data = _ser(output)
        result, remaining = _deser(data)
        assert len(result.script) == MAX_SHIELDED_OUTPUT_SCRIPT_SIZE


# ============================================================================
# Legacy verifier shielded routing
# ============================================================================

class TestLegacyVerifierShieldedRouting:
    def test_verify_sigops_input_handles_shielded_output_spending(self) -> None:
        ...
        # MagicMock dispatches `.resolve_spent_output(0)` through the unified shielded-aware
        # lookup; here we wire it to return the shielded output directly.
    ...
    def test_script_eval_handles_shielded_output(self) -> None:
        ...
        try:
            ...
            pytest.fail(f"IndexError should not be raised: {e}")
        except Exception:
            pass  # Other exceptions expected with dummy data


# ============================================================================
# Verify_sum skipped for shielded transactions
# ============================================================================
...
class TestFFIErrorWrapping:
    def test_invalid_commitment_bytes_raises_tx_validation_error(self) -> None:
        ...
        # Should raise InvalidShieldedOutputError (from curve point validation)
...
class TestTrivialCommitmentProtection:
    ...
    def test_single_shielded_output_always_rejected(self) -> None:
        """Single shielded output is rejected regardless of input type (uniform >=2 rule)."""
        verifier = _make_verifier()
        output = _make_amount_shielded()
        tx = _mock_tx([output])
...
class TestFeatureGate:
    def test_feature_activation_mode_blocks_before_activation(self) -> None:
        """params.features.shielded_transactions=False should be rejected.
    ...
    def test_feature_activation_mode_allows_after_activation(self) -> None:
        """params.features.shielded_transactions=True should be allowed."""
        from hathor.transaction import Transaction
        from hathor.verification.verification_service import VerificationService
    ...
    def test_gate_uses_params_not_settings(self) -> None:
        ...
        # verify() relies on verify_basic's feature gate (no redundant check needed)
...
# ============================================================================
# Zero-value panic guard
# ============================================================================

class TestZeroValuePanicGuard:
    def test_balance_skips_zero_value_transparent_entries(self) -> None:
        ...
        from hathorlib.crypto.shielded import verify_balance
...
class TestBufferTruncation:
    def test_truncated_commitment_rejected_during_deserialization(self) -> None:
        """Short buffer for commitment → SerializationError (out of data)."""
    ...
    def test_truncated_range_proof_rejected(self) -> None:
        """Short buffer claiming more range_proof bytes than available → SerializationError."""
...
# ============================================================================
# Verify_tokens with shielded
# ============================================================================

class TestVerifyTokensShielded:
...
# ============================================================================
# AR-002: sign_all_inputs with shielded outputs
# ============================================================================

class TestSignAllInputsShielded:
    def test_sign_all_inputs_resolves_shielded_output(self) -> None:
        """sign_all_inputs should use resolve_spent_output, not pi.outputs[index]."""
        # We test by verifying the code uses resolve_spent_output
        import inspect

        from hathor.dag_builder.vertex_exporter import VertexExporter

        source = inspect.getsource(VertexExporter.sign_all_inputs)
        assert 'resolve_spent_output' in source
        assert 'pi.outputs[txin.index]' not in source


# ============================================================================
# AR-003: Deduplicated normalize_token_uid
# ============================================================================

class TestNormalizeTokenUidDedup:
    def test_normalize_token_uid_is_public(self) -> None:
        """normalize_token_uid should be importable from asset_tag."""
        from hathorlib.crypto.shielded.asset_tag import normalize_token_uid

        # 1-byte HTR → padded to 32
        result = normalize_token_uid(b'\x00')
        assert len(result) == 32
        assert result == b'\x00' * 32

    def test_normalize_token_uid_32_bytes_passthrough(self) -> None:
        """32-byte UIDs pass through unchanged."""
        from hathorlib.crypto.shielded.asset_tag import normalize_token_uid
        uid = b'\x01' * 32
        assert normalize_token_uid(uid) == uid

    def test_normalize_token_uid_invalid_length_raises(self) -> None:
        """Invalid length raises ValueError."""
        from hathorlib.crypto.shielded.asset_tag import normalize_token_uid
        with pytest.raises(ValueError, match='invalid token UID length'):
            normalize_token_uid(b'\x00' * 5)

    def test_verifier_normalize_wraps_in_invalid_shielded_output_error(self) -> None:
        """Verifier's _normalize_token_uid wraps ValueError in InvalidShieldedOutputError."""
        verifier = _make_verifier()
        with pytest.raises(InvalidShieldedOutputError, match='invalid token UID length'):
            verifier._normalize_token_uid(b'\x00' * 5)


# ============================================================================
# AR-006: Ephemeral pubkey helpers
# ============================================================================

class TestEphemeralPubkeyHelpers:
    def test_write_ephemeral_pubkey_absent(self) -> None:
        """Empty pubkey serializes to zeros in the output bytes."""
        from hathorlib.transaction.shielded_tx_output import EPHEMERAL_PUBKEY_SIZE
        output = _make_amount_shielded(ephemeral_pubkey=b'')
        data = _ser(output)
        # Last EPHEMERAL_PUBKEY_SIZE bytes should be all zeros
        assert data[-EPHEMERAL_PUBKEY_SIZE:] == b'\x00' * EPHEMERAL_PUBKEY_SIZE

    def test_write_ephemeral_pubkey_present(self) -> None:
        """Non-empty pubkey serializes as-is in the output bytes."""
        from hathorlib.transaction.shielded_tx_output import EPHEMERAL_PUBKEY_SIZE
        pubkey = b'\x02' + os.urandom(32)
        output = _make_amount_shielded(ephemeral_pubkey=pubkey)
        data = _ser(output)
        # Last EPHEMERAL_PUBKEY_SIZE bytes should be the actual pubkey
        assert data[-EPHEMERAL_PUBKEY_SIZE:] == pubkey

    def test_read_ephemeral_pubkey_absent(self) -> None:
        """All-zero ephemeral pubkey deserializes to empty."""
        output = _make_amount_shielded(ephemeral_pubkey=b'')
        data = _ser(output)
        result, _ = _deser(data)
        assert result.ephemeral_pubkey is None

    def test_read_ephemeral_pubkey_present(self) -> None:
        """Non-zero ephemeral pubkey deserializes correctly."""
        pubkey = b'\x02' + os.urandom(32)
        output = _make_amount_shielded(ephemeral_pubkey=pubkey)
        data = _ser(output)
        result, _ = _deser(data)
        assert result.ephemeral_pubkey == pubkey

    def test_serialization_roundtrip_with_helpers(self) -> None:
        """serialize/deserialize roundtrip still works after helper refactor."""
        output = _make_amount_shielded()
        data = _ser(output)
        result, remaining = _deser(data)
        assert result.commitment == output.commitment
        assert result.script == output.script
        assert remaining == b''


# ============================================================================
# AD-002: Always emit has_shielded
# ============================================================================

class TestAlwaysEmitHasShielded:
    def test_address_balance_always_emits_has_shielded(self) -> None:
        """address_balance.py should always include has_shielded, not conditionally."""
        import inspect

        from hathor.wallet.resources.thin_wallet.address_balance import AddressBalanceResource

        source = inspect.getsource(AddressBalanceResource.render_GET)
        # Old code was: if has_shielded: data['has_shielded'] = True
        # New code: data['has_shielded'] = has_shielded
        assert "data['has_shielded'] = has_shielded" in source
        assert "if has_shielded:" not in source.split("data['has_shielded']")[0].split('\n')[-1]

    def test_address_search_always_emits_has_shielded(self) -> None:
        """address_search.py should always include has_shielded, not conditionally."""
        import inspect

        from hathor.wallet.resources.thin_wallet.address_search import AddressSearchResource

        source = inspect.getsource(AddressSearchResource.render_GET)
        assert "data['has_shielded'] = has_shielded" in source


# ============================================================================
# AD-004: Input type discriminator
# ============================================================================

class TestInputTypeDiscriminator:
    def test_transparent_inputs_have_type_field(self) -> None:
        """to_json_extended should add type='transparent' to regular input entries."""
        import inspect

        from hathor.transaction.base_transaction import BaseTransaction, _shielded_output_to_json

        source = inspect.getsource(BaseTransaction.to_json_extended)
        assert "['type'] = 'transparent'" in source
        # Shielded inputs are serialized via the _shielded_output_to_json helper, which tags them.
        assert "'type': 'shielded'" in inspect.getsource(_shielded_output_to_json)


# ============================================================================
# AR-012: No bare except Exception in DAG builder
# ============================================================================

class TestDagBuilderNoBareExcept:
    def test_no_bare_except_in_get_recipient_pubkey(self) -> None:
        """_get_recipient_pubkey_from_script should catch TypeError, not Exception."""
        import inspect

        from hathor.dag_builder.vertex_exporter import VertexExporter

        source = inspect.getsource(VertexExporter._get_recipient_pubkey_from_script)
