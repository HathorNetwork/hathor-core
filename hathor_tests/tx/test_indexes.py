...
class BaseIndexesTest(unittest.TestCase):
    ...
    def test_tx_tips_voided(self):
        ...
        output1 = WalletOutputInfo(address=decode_address(address1), value=TokenAmount.from_v1(123), timelock=None)
        output2 = WalletOutputInfo(address=decode_address(address2), value=TokenAmount.from_v1(234), timelock=None)
        output3 = WalletOutputInfo(address=decode_address(address3), value=TokenAmount.from_v1(345), timelock=None)
    ...
    def test_utxo_index_genesis(self):
        ...
        expected_genesis_utxos = [
            UtxoIndexItem(
                ...
                amount=TokenAmount.from_v1(self._settings.GENESIS_TOKEN_ATOMIC_UNITS),
                timelock=None,
        ...
        self.assertEqual(
            list(utxo_index.iter_utxos(address=GENESIS_ADDRESS_B58, token_uid=self._settings.HATHOR_TOKEN_UID,
                                       target_amount=TokenAmount.from_v1(self._settings.GENESIS_TOKEN_MAIN_UNITS),
    ...
    def test_utxo_index_reorg(self):
        ...
        def check_utxos(*args):
            ...
            actual = list(utxo_index.iter_utxos(address=address, target_amount=TokenAmount.from_v1(9999999)))
            expected = [
                UtxoIndexItem(
                    ...
                    index=index,
                    address=address,
                    amount=TokenAmount.from_v1(amount),
                    timelock=None,
    ...
    def test_utxo_index_simple(self):
        ...
        self.assertEqual(
            list(utxo_index.iter_utxos(address=address, target_amount=TokenAmount.from_v1(1))),
            []
        )
        ...
        self.assertEqual(
            ...
            [
                UtxoIndexItem(
                    ...
                    index=0,
                    address=address,
                    amount=TokenAmount.from_v1(6400),
                    timelock=None,
        ...
        self.assertEqual(
            list(utxo_index.iter_utxos(address=address, target_amount=TokenAmount.from_v1(6500))),
            [
                UtxoIndexItem(
                    token_uid=self._settings.HATHOR_TOKEN_UID,
                    tx_id=b.hash,
        ...
        self.assertEqual(
            list(utxo_index.iter_utxos(address=address, target_amount=TokenAmount.from_v1(25600))),
            [
                UtxoIndexItem(
                    token_uid=self._settings.HATHOR_TOKEN_UID,
                    tx_id=b.hash,
        ...
        self.assertEqual(
            list(utxo_index.iter_utxos(address=address, target_amount=TokenAmount.from_v1(30000))),
            [
                UtxoIndexItem(
                    token_uid=self._settings.HATHOR_TOKEN_UID,
                    tx_id=b.hash,
    ...
    def test_utxo_index_limits(self):
        ...
        for txs_window in iwindows(txs_and_values, 3):
            ...
            actual = list(utxo_index.iter_utxos(address=address, target_amount=TokenAmount.from_v1(target_amount)))
            if _debug:
                print('expected = [')
                for x in expected:
                    print(f'\t{x!r},')
        ...
        actual = list(utxo_index.iter_utxos(address=address, target_amount=TokenAmount.from_v1(sum(range(301)))))
        expected = [
            UtxoIndexItem(
                token_uid=self._settings.HATHOR_TOKEN_UID,
                tx_id=tx.hash,
    ...
    def test_utxo_index_after_push_tx(self):
        ...
        tx1 = Transaction(
            ...
            outputs=[TxOutput(TokenAmount.from_v1(6400), P2PKH.create_output_script(decode_address(address1)))],
        ...
        self.assertEqual(
            list(utxo_index.iter_utxos(address=address1, target_amount=TokenAmount.from_v1(6400))),
            [
                UtxoIndexItem(
                    token_uid=self._settings.HATHOR_TOKEN_UID,
                    tx_id=tx1.hash,
    ...
    def test_utxo_index_last(self):
        ...
        tx1 = Transaction(
            ...
            weight=1.0,
            inputs=[TxInput(blocks[0].hash, 0, b'')],
            outputs=[TxOutput(TokenAmount.from_v1(change_value), P2PKH.create_output_script(decode_address(address))),
                     TxOutput(TokenAmount.from_v1(transfer_value),
                              P2PKH.create_output_script(decode_address(address1)))],
            parents=list(self.manager.get_new_tx_parents()),
            storage=self.tx_storage,
        )
        tx1.inputs[0].data = P2PKH.create_input_data(
        ...
        self.assertEqual(
            list(utxo_index.iter_utxos(address=address1, target_amount=TokenAmount.from_v1(transfer_value))),
            [
                UtxoIndexItem(
                    token_uid=self._settings.HATHOR_TOKEN_UID,
                    tx_id=tx1.hash,
                    index=1,
                    address=address1,
                    amount=TokenAmount.from_v1(transfer_value),
                    timelock=None,
                    heightlock=None,
                )
            ]
        ...
        self.assertEqual(
            list(utxo_index.iter_utxos(address=address, target_amount=TokenAmount.from_v1(change_value))),
            [
                UtxoIndexItem(
                    token_uid=self._settings.HATHOR_TOKEN_UID,
                    tx_id=tx1.hash,
                    index=0,
                    address=address,
                    amount=TokenAmount.from_v1(change_value),
                    timelock=None,
                    heightlock=None,
                )
            ]
        ...
        self.assertEqual(
            list(utxo_index.iter_utxos(address=address1, target_amount=TokenAmount.from_v1(1))),
            [
                UtxoIndexItem(
                    token_uid=self._settings.HATHOR_TOKEN_UID,
                    tx_id=tx1.hash,
