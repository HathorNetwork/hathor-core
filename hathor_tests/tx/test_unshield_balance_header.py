...
from hathor.transaction.vertex_parser._unshield_balance_header import deserialize_unshield_balance_header
...
class TestUnshieldBalanceHeaderUnit:
    ...
    def test_construction_rejects_wrong_size(self) -> None:
        with pytest.raises(ValueError, match='must be 32 bytes'):
            UnshieldBalanceHeader(excess_blinding_factor=b'\x01' * 16)
        with pytest.raises(ValueError, match='must be 32 bytes'):
            UnshieldBalanceHeader(excess_blinding_factor=b'\x01' * 33)

    def test_construction_accepts_32_bytes(self) -> None:
        scalar = os.urandom(EXCESS_BLINDING_FACTOR_SIZE)
        header = UnshieldBalanceHeader(excess_blinding_factor=scalar)
    ...
    def test_header_id_byte(self) -> None:
        ...
        # The header serializes with that id as its first byte.
        raw = UnshieldBalanceHeader(excess_blinding_factor=b'\x00' * EXCESS_BLINDING_FACTOR_SIZE).serialize()
        assert raw[0:1] == b'\x13'
    ...
    def test_deserialize_rejects_short_buffer(self) -> None:
        ...
        with pytest.raises(InvalidShieldedOutputError, match='malformed'):
            deserialize_unshield_balance_header(Deserializer.build_bytes_deserializer(short))
    ...
    def test_deserialize_rejects_wrong_header_id(self) -> None:
        ...
        with pytest.raises(InvalidShieldedOutputError, match='unexpected header id'):
            deserialize_unshield_balance_header(Deserializer.build_bytes_deserializer(raw))
    ...
    def test_sighash_binds_scalar(self) -> None:
        ...
        mutated[0] ^= 0xFF
        header_mut = UnshieldBalanceHeader(excess_blinding_factor=bytes(mutated))
...
class TestVertexVerifierAllowedHeaders:
    ...
    def _make_tx_with_unshield_header(self) -> MagicMock:
        ...
        tx.headers = [UnshieldBalanceHeader(excess_blinding_factor=os.urandom(32))]
        return tx
