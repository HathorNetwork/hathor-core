...
class FeeTokenTest(unittest.TestCase):
    ...
    def test_fee_token_melt(self) -> None:
        ...
        outputs = [
            # New token amount
            TxOutput(TokenAmount.from_v1(100), script, 1),
            TxOutput(TokenAmount.from_v1(100), script, 1),
            TxOutput(TokenAmount.from_v1(100), script, 1),
            TxOutput(TokenAmount.from_v1(100), script, 1),
            # Melt authority
            TxOutput(TokenAmount.from_v1(TxOutput.TOKEN_MELT_MASK), script, 0b10000001),
            TxOutput(TokenAmount.from_v1(htr_amount - 4), script, 0)
        ]
        ...
        fee_header = FeeHeader(
            ...
            tx=tx2,
            fees=[FeeHeaderEntry(token_index=0, amount=TokenAmount.from_v1(4))]
        )
        ...
        self.assertEqual(tx_fee, TokenAmount.from_v1(4))
        ...
        outputs = [
            TxOutput(TokenAmount.from_v1(100), script, 2),
            TxOutput(TokenAmount.from_v1(100), script, 2),
            TxOutput(TokenAmount.from_v1(100), script, 2),
            TxOutput(TokenAmount.from_v1(96-5), script, 0)
        ]
        ...
        fee_header = FeeHeader(
            settings=self._settings,
            tx=tx3,
            fees=[FeeHeaderEntry(token_index=0, amount=TokenAmount.from_v1(5))],
        )
        ...
        self.assertEqual(tx3_fee, TokenAmount.from_v1(5))
    ...
    def test_fee_token_melt_without_authority(self) -> None:
        ...
        outputs = [
            ...
            TxOutput(TokenAmount.from_v1(new_token_amount), script, 1),
            TxOutput(TokenAmount.from_v1(4), script, 0)
        ]
        ...
        fee_header = FeeHeader(
            settings=self._settings,
            tx=tx2,
            fees=[FeeHeaderEntry(token_index=0, amount=TokenAmount.from_v1(1))],
        )
        tx2.headers.append(fee_header)
        ...
        nc_storage = self.manager.get_nc_block_storage(self.manager.tx_storage.get_best_block())
        tx_fee = tx2.get_complete_token_info(nc_storage).calculate_fee(self.manager._settings)
        self.assertEqual(tx_fee, TokenAmount.from_v1(1))
        self.assertEqual(tx_fee, fee_header.total_fee_amount())
    ...
    def test_fee_token_melt_paid_with_deposit(self) -> None:
        ...
        outputs = [
            ...
            TxOutput(TokenAmount.from_v1(400), script, 2)
        ]
        ...
        fee_header = FeeHeader(
            settings=self._settings,
            tx=tx2,
            fees=[FeeHeaderEntry(token_index=2, amount=TokenAmount.from_v1(100))],
        )
        tx2.headers.append(fee_header)
    ...
    def test_fee_and_deposit_token_melt_paid_with_deposit(self) -> None:
        ...
        htr_change_value = get_deposit_token_withdraw_amount(
            self.manager._settings, TokenAmount.from_v1(dbt_melt_amount)
        )
        # 200 dbt -> 2 htr
        self.assertEqual(htr_change_value, TokenAmount.from_v1(2))
        outputs = [
            # New token amount
            TxOutput(TokenAmount.from_v1(new_token_amount), script, 1),
            # Melt authority
            ...
            TxOutput(TokenAmount.from_v1(200), script, 2)
        ]
    ...
    def test_fee_token_tx_paid_with_htr_and_deposit(self) -> None:
        ...
        outputs = [
            ...
            TxOutput(TokenAmount.from_v1(300), script, 2),  # 500 - 200
            TxOutput(TokenAmount.from_v1(2), script)  # 5 - 3
        ]
        ...
        fee_header = FeeHeader(
            ...
            tx=tx2,
            fees=[
                FeeHeaderEntry(token_index=0, amount=TokenAmount.from_v1(3)),
                FeeHeaderEntry(token_index=2, amount=TokenAmount.from_v1(200))
            ],
        )
        tx2.headers.append(fee_header)
        nc_storage = self.manager.get_nc_block_storage(self.manager.tx_storage.get_best_block())
        tx_fee = tx2.get_complete_token_info(nc_storage).calculate_fee(self.manager._settings)
        self.assertEqual(tx_fee, TokenAmount.from_v1(5))
        self.assertEqual(tx_fee, fee_header.total_fee_amount())
    ...
    def test_fee_token_mint(self) -> None:
        ...
        outputs = [
            # Token minted output
            TxOutput(TokenAmount.from_v1(mint_amount), script, 1),
        ...
        tx_fee = tx2.get_complete_token_info(nc_storage).calculate_fee(self.manager._settings)
        self.assertEqual(tx_fee, TokenAmount.from_v1(1))
        self.assertEqual(fee_header.total_fee_amount(), TokenAmount.from_v1(1))
    ...
    def test_fee_token_tx_without_paying(self) -> None:
        ...
        outputs = [
            # Token output
            TxOutput(TokenAmount.from_v1(250), script, 1),
            # Token output
            TxOutput(TokenAmount.from_v1(250), script, 1),
        ]
        ...
        nc_storage = self.manager.get_nc_block_storage(self.manager.tx_storage.get_best_block())
        tx_fee = tx2.get_complete_token_info(nc_storage).calculate_fee(self.manager._settings)
        self.assertEqual(tx_fee, TokenAmount.from_v1(2))
        ...
        expected_msg = (
            ...
            f'(amount=0, expected={TokenAmount.from_v1(2).normalized()})'
        )
    ...
    def test_fee_token_burn_authority(self) -> None:
        ...
        nc_storage = self.manager.get_nc_block_storage(self.manager.tx_storage.get_best_block())
        tx_fee = tx2.get_complete_token_info(nc_storage).calculate_fee(self.manager._settings)
        self.assertEqual(tx_fee, TokenAmount.zero())
    ...
    def test_verify_token_info(self) -> None:
        ...
        outputs = [
            # mint output
            TxOutput(TokenAmount.from_v1(500), script, 0b00000001),
            # authority outputs
            TxOutput(TokenAmount.from_v1(TxOutput.TOKEN_MINT_MASK), script, 0b10000001),
            TxOutput(TokenAmount.from_v1(TxOutput.TOKEN_MELT_MASK), script, 0b10000001),
            # deposit output
