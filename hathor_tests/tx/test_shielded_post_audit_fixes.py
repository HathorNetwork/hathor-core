...
# ---------------------------------------------------------------------------
# C-001: Wallet exception handler must be narrow
# ---------------------------------------------------------------------------
...
class TestC006ShieldedVerificationLogging:
    ...
    def test_shielded_verifier_has_logger(self):
        """The TransactionVerifier must have a logger attribute."""
        from hathor.verification.transaction_verifier import TransactionVerifier

        settings = MagicMock()
        verifier = TransactionVerifier(settings=settings, daa_factory=MagicMock(), feature_service=MagicMock())
        assert hasattr(verifier, 'log'), (
            "TransactionVerifier must have a 'log' attribute for structured logging"
        )
