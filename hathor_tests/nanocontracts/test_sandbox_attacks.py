...
Note: This file focuses on STATEMENT LIMITS which work reliably.
...
from hathor.nanocontracts.metered_exec import BLUEPRINT_FILENAME, MeteredExecutor
from hathor_tests.nanocontracts.blueprints.unittest import BlueprintTestCase


def _ensure_sandbox_ready() -> None:
    """Ensure sandbox is properly configured for MeteredExecutor tests.

    This is needed because the sandbox suspend counter and filename registrations
    can get out of sync between tests running in the same process.
    """
    # First, ensure sandbox is fully unsuspended (counter = 0)
    # by resuming until suspended returns False
    while sys.sandbox.suspended:
        sys.sandbox.resume()

    # Reset to default limits and register the blueprint filename
    MeteredExecutor._sandbox_initialized = False  # Force re-initialization
    MeteredExecutor.reset_sandbox_limits()
    MeteredExecutor._ensure_sandbox_initialized()


class SandboxStatementLimitTestCase(BlueprintTestCase):
    """Test statement limit enforcement in various scenarios."""
    ...
    def test_loop_blocked_with_low_statement_limit(self) -> None:
        """Loop blocked by low statement limit (safe test)."""
        code = '''
...
'''
        # Register and create contract with default limits
...
'''
    ...
    def test_syscalls_do_not_count_as_statements(self) -> None:
        """Verify syscalls do not count towards statement limits."""
        code = '''
from hathor import Blueprint, Context, public
...
'''
        ...
        # Set a low statement limit
        MeteredExecutor.max_statements = 500
        MeteredExecutor._apply_sandbox_limits()
...
class CrossContractStatementLimitTestCase(BlueprintTestCase):
    """Test statement limit enforcement across contract calls."""
    ...
    def test_cross_contract_statement_count_not_reset(self) -> None:
        """
        Verify statement count is NOT reset when calling another contract.
...
__blueprint__ = WorkerBlueprint
'''
        # Register and create contracts with default limits
        blueprint_id = self._register_blueprint_contents(
        ...
        # First, verify each contract can do 150 iterations alone (under 500 limit)
        ...
        # Total = 600 statements, should exceed the 500 limit
        with self.assertRaises(NCFail) as cm:
    ...
    def test_chain_of_three_contracts(self) -> None:
        """Test statement counting across a chain of 3 contracts (safe with low limits)."""
        code = '''
from hathor import Blueprint, Context, public
from hathor import ContractId
...
class MultipleOCBSandboxTestCase(BlueprintTestCase):
    ...
    def test_two_different_blueprints_share_statement_count(self) -> None:
        """Test that two contracts from DIFFERENT blueprints share statement count."""
        code_blueprint1 = '''
from hathor import Blueprint, Context, public
from hathor import ContractId
...
'''
    ...
    def test_reentrant_calls_share_statement_count(self) -> None:
        """Test that reentrant calls (A calls B calls A) share statement count."""
        code = '''
from hathor import Blueprint, Context, public
from hathor import ContractId
...
__blueprint__ = ReentrantBlueprint
'''
        # Register and create contracts with default limits
        blueprint_id = self._register_blueprint_contents(
            StringIO(code),
