...
class TestNanoFeatureActivation(unittest.TestCase):
    ...
    async def test_activation(self) -> None:
        ...
        with pytest.raises(InvalidNewTransaction, match=msg):
            self.vertex_handler.on_new_mempool_transaction(op_v2_b)
        ...
        with pytest.raises(InvalidNewTransaction, match=msg):
            self.vertex_handler.on_new_mempool_transaction(nc1)
        ...
        with pytest.raises(InvalidNewTransaction, match=msg):
            self.vertex_handler.on_new_mempool_transaction(ocb1)
        ...
        with pytest.raises(InvalidNewTransaction, match=msg):
            self.vertex_handler.on_new_mempool_transaction(fbt)
        ...
        self.vertex_handler.on_new_mempool_transaction(fee_tx)
        assert fee_tx.get_metadata().validation.is_valid()
        assert fee_tx.get_metadata().voided_by is None
