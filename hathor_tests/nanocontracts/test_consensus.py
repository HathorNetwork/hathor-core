...
class NCConsensusTestCase(SimulatorTestCase):
    ...
    def setUp(self):
        ...
        trigger = StopAfterMinimumBalance(self.wallet, self.token_uid, TokenAmount.from_v1(1))
    ...
    def test_nc_consensus_success(self, *, is_custom_token: bool = False) -> None:
        ...
        nc_storage = self.manager.get_best_block_nc_storage(nc_id)
        self.assertEqual(
            Balance(value=deposit_amount.to_balance(), can_mint=False, can_melt=False),
        ...
        tx2 = Transaction(
            outputs=[TxOutput(TokenAmount.from_v1(1), b'', _output_token_index)],
        ...
        nc_storage = self.manager.get_best_block_nc_storage(nc_id)
        self.assertEqual(
            Balance(value=(deposit_amount - TokenAmount.from_v1(1)).to_balance(), can_mint=False, can_melt=False),
            nc_storage.get_balance(self.token_uid)
        )

        # Make a withdrawal of the remainder.

        remainder = TokenAmount.from_v1(deposit_amount.raw() - 2)
        tx3 = Transaction(
        ...
        nc_storage = self.manager.get_best_block_nc_storage(nc_id)
        self.assertEqual(
            Balance(value=TokenAmount.from_v1(1).to_balance(), can_mint=False, can_melt=False),
            nc_storage.get_balance(self.token_uid),
        )
        ...
        tx4 = Transaction(
            outputs=[TxOutput(TokenAmount.from_v1(2), b'', _output_token_index)],
            timestamp=int(self.manager.reactor.seconds()),
        )
        tx4.tokens = _tokens
        ...
        self.assertEqual(
            Balance(value=TokenBalance(), can_mint=False, can_melt=False),
            nc_storage.get_balance(self.token_uid),
        )
    ...
    def test_nc_consensus_failure_voided_by_propagation(self):
        ...
        _outputs = [
            WalletOutputInfo(address, TokenAmount.from_v1(1), None),
            WalletOutputInfo(address, TokenAmount.from_v1(1), None),
        ]
    ...
    def test_nc_consensus_reorg(self):
        ...
        withdrawal_amount_1 = TokenAmount.from_v1(deposit_amount_1.raw() - 100)
        ...
        self.assertEqual(
            Balance(
                value=(deposit_amount_1 - withdrawal_amount_1).to_balance(),
                can_mint=False,
                can_melt=False,
            ),
            nc_storage.get_balance(self.token_uid)
        ...
        nc_storage = self.manager.get_best_block_nc_storage(nc_id)
        self.assertEqual(
            Balance(value=deposit_amount_2.to_balance(), can_mint=False, can_melt=False),
            nc_storage.get_balance(self.token_uid)
        )
    ...
    def test_nc_consensus_reorg_fail_before_reorg(self):
        ...
        withdrawal_amount_1 = TokenAmount.from_v1(deposit_amount_2.raw() - 100)
        tx11 = Transaction(outputs=[TxOutput(withdrawal_amount_1, b'', 0)])
        tx11 = self._gen_nc_tx(nc_id, 'withdraw', [], nc=tx11, address=address1, nc_actions=[
            NanoHeaderAction(
                type=NCActionType.WITHDRAWAL,
        ...
        nc_storage = self.manager.get_best_block_nc_storage(nc_id)
        self.assertEqual(
            Balance(value=deposit_amount_1.to_balance(), can_mint=False, can_melt=False),
            nc_storage.get_balance(self.token_uid)
        )
        ...
        self.assertEqual(
            Balance(
                value=(deposit_amount_2 - withdrawal_amount_1).to_balance(),
                can_mint=False,
                can_melt=False,
            ),
            nc_storage.get_balance(self.token_uid)
    ...
    def _prepare_nc_consensus_conflict(self, *, conflict_with_nano: bool) -> tuple[Transaction, ...]:
        ...
        address = self.wallet.get_unused_address_bytes()
        _outputs = [
            WalletOutputInfo(address, TokenAmount.from_v1(10), None),
        ]
        ...
        tx1.outputs[0].value = TokenAmount.from_v1(3)
        tx1 = self._gen_nc_tx(nc.hash, 'deposit', [], nc=tx1, nc_actions=[
            NanoHeaderAction(
                type=NCActionType.DEPOSIT,
                token_index=0,
                amount=TokenAmount.from_v1(tx0.outputs[0].value.raw() - 3),
            )
        ])
        self.manager.cpu_mining_service.resolve(tx1)
