...
class SandboxAPIConfigLoaderTest(unittest.TestCase):
    ...
    def test_init_with_none(self) -> None:
        """Test initializing with no config file."""
    ...
    def test_init_with_nonexistent_file(self) -> None:
        """Test initializing with a nonexistent file."""
        loader = SandboxAPIConfigLoader('/path/that/does/not/exist.yaml')
        self.assertFalse(loader.config.enabled)  # DISABLED_CONFIG
...
"""
        ...
        try:
            loader = SandboxAPIConfigLoader(temp_path)
...
"""
        ...
        try:
            loader = SandboxAPIConfigLoader(temp_path_1)
            self.assertIsNotNone(loader.config)
            assert loader.config is not None
            self.assertEqual(loader.config.max_operations, 1000000)
...
"""
    ...
    def test_set_file_to_none_disables(self) -> None:
        """Test set_file(None) disables the config."""
        yaml_content = """
api_view:
  enabled: true
  max_operations: 1000000
...
"""
        ...
        try:
            # Test with string
            loader_str = SandboxAPIConfigLoader(temp_path)
            self.assertIsNotNone(loader_str.config)

            # Test with Path
            loader_path = SandboxAPIConfigLoader(Path(temp_path))
