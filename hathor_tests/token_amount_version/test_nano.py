...
from hathorlib.serialization.encoding.int import encode_int
...
class TestNano(BlueprintTestCase):
    ...
    def test_new_serialization(self) -> None:
        ...
        encode_int(se, 64, length=4, signed=True)
        assert x_raw == b'\x01' + bytes(se.finalize()) == b'\x01\x00\x00\x00\x40'

        y_key = storage._to_attr_key(b'y')
        y_raw = storage._trie.get(bytes(y_key))
        se = Serializer.build_bytes_serializer()
        encode_int(se, 64, length=4, signed=False)
        assert y_raw == b'\x01' + bytes(se.finalize()) == b'\x01\x00\x00\x00\x40'
