...
class SyncMethodsTestCase(unittest.TestCase):
    ...
    def test_simple_double_spending(self) -> None:
        ...
        outputs = []
        outputs.append(
            WalletOutputInfo(address=decode_address(address), value=TokenAmount.from_v1(value), timelock=None))
    ...
    def test_double_spending_propagation(self) -> None:
        ...
        value = 100
        outputs = [WalletOutputInfo(address=address, value=TokenAmount.from_v1(int(value)), timelock=None)]
        ...
        address = self.manager1.wallet.get_unused_address_bytes()
        value = 500
        tx_total_value = sum((txout.value for txout in tx1.outputs), start=TokenAmount.zero())
        outputs = [WalletOutputInfo(address=address, value=TokenAmount.from_v1(value), timelock=None),
                   WalletOutputInfo(address=address, value=tx_total_value - TokenAmount.from_v1(500),
                                    timelock=None)]
        self.clock.advance(1)
