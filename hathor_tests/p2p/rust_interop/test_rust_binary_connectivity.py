...
A full Python node (``run_node --testnet``) and the Rust ``hathor-next`` binary talk over a real
...
def _find_rust_binary() -> str | None:
    """Locate the `hathor-next` binary: `HATHOR_NEXT_BIN`, then a prebuilt target binary.
    ...
    simply skips unless a prebuilt binary or `HATHOR_NEXT_BIN` is provided. Returns the path, or
    `None` when no binary is available.
    """
    override = os.environ.get('HATHOR_NEXT_BIN')
    if override:
        return override if Path(override).exists() else None

    for profile in ('debug', 'release'):
        candidate = _RUST_WORKSPACE / 'target' / profile / 'hathor-next'
        if candidate.exists():
    ...
    if result.returncode != 0:
        return None
    built = _RUST_WORKSPACE / 'target' / 'debug' / 'hathor-next'
...
def rust_binary() -> str:
    ...
    if binary is None:
        pytest.skip(
            'hathor-next binary unavailable; build it (cargo build --bin hathor-next) or set '
            'HATHOR_NEXT_BIN. Not auto-built under CI.'
        )
    return binary
