...
"""Cross-implementation serialization parity between the Rust port (`htr_core`) and the Python
reference.

The Rust codecs are exposed through the `htr_core` PyO3 extension (crate `htr-core-py`):
...
class RustVertexParityTest(unittest.TestCase):
    ...
    def _assert_vertex_roundtrip(self, name: str, vertex_bytes: bytes) -> None:
        reencoded = htr_core.vertex_decode_encode(vertex_bytes)
        self.assertEqual(
            reencoded,
            vertex_bytes,
    ...
    def test_malformed_vertex_raises(self) -> None:
        for bad in [b'', b'\x00', b'\x00\x00', b'\xff' * 8, b'not a vertex at all']:
            with self.assertRaises(ValueError):
                htr_core.vertex_decode_encode(bad)
...
class RustMessageParityTest(unittest.TestCase):
    ...
    def _assert_message_roundtrip(self, state: str, line: str) -> str:
        rendered = htr_core.message_reencode(state, line)
        # Re-rendering is idempotent: feeding the canonical form back yields the same string.
        self.assertEqual(
            htr_core.message_reencode(state, rendered),
            rendered,
    ...
    def test_embedded_vertex_in_message_roundtrips(self) -> None:
        ...
        rendered = htr_core.message_reencode('ready', f'BLOCKS {_b64(block)}')
    ...
    def test_out_of_state_messages_are_rejected(self) -> None:
        ...
        with self.assertRaises(ValueError):
            htr_core.message_reencode('hello', 'PING x')
        with self.assertRaises(ValueError):
            htr_core.message_reencode('peer-id', 'GET-PEERS')
        with self.assertRaises(ValueError):
            htr_core.message_reencode('ready', 'HELLO {}')

    def test_unknown_state_is_rejected(self) -> None:
        with self.assertRaises(ValueError):
            htr_core.message_reencode('not-a-state', 'GET-PEERS')
    ...
    def test_reference_messages_not_modeled_by_rust(self) -> None:
        ...
        with self.assertRaises(ValueError):
            htr_core.message_reencode('ready', f'BEST-BLOCK {{"block": "{_HASH_A}", "height": 5}}')
        with self.assertRaises(ValueError):
            htr_core.message_reencode('ready', 'GET-MEMPOOL')
