...
def test_tokenize_transparent_output_still_works() -> None:
    # On this branch `out[...]` attrs are tokenized into a dict (bracket-stripped keys),
    # unlike the list form used by the `sout[...]` shielded path.
    tokens = list(tokenize('tx1.out[0] = 100 HTR [wallet1]'))
    assert tokens == [(TokenType.OUTPUT, ('tx1', 0, 100, 'HTR', {'wallet1': 1}))]
