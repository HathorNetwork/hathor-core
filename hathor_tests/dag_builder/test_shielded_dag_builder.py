...
class ShieldedDAGBuilderTestCase(unittest.TestCase):
    ...
    def test_header_serialization_roundtrip(self) -> None:
        ...
        # Deserialize through the framework free-function (the real hathor-core path).
        deserializer = Deserializer.build_bytes_deserializer(data)
        restored_outputs = deserialize_shielded_outputs_header(deserializer)
        self.assertEqual(len(bytes(deserializer.read_all())), 0)
        self.assertEqual(len(restored_outputs), len(header.shielded_outputs))

        for orig, rest in zip(header.shielded_outputs, restored_outputs):
