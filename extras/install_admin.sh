...
git clone git@gitlab.com:HathorNetwork/admin-frontend.git || exit 1
...
sudo certbot certonly --standalone --preferred-challenges http -d ${NODE_HOST} --pre-hook "/etc/init.d/nginx stop" --post-hook "/etc/init.d/nginx start"
