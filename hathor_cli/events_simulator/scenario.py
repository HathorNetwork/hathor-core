...
def simulate_single_chain_blocks_and_transactions(
    ...
    tx = gen_new_tx(manager, address, TokenAmount.from_v1(1000))
    tx.weight = manager.daa_factory.minimum_tx_weight(tx)
    tx.update_hash()
    assert manager.propagate_tx(tx)
    simulator.run(60)

    tx = gen_new_tx(manager, address, TokenAmount.from_v1(2000))
    tx.weight = manager.daa_factory.minimum_tx_weight(tx)
    tx.update_hash()
    assert manager.propagate_tx(tx)
    simulator.run(60)
...
def simulate_invalid_mempool_transaction(simulator: 'Simulator', manager: 'HathorManager') -> Optional['DAGArtifacts']:
    ...
    assert balance_per_address[address] == TokenAmount.from_v1(6400)
    tx = gen_new_tx(manager, address, TokenAmount.from_v1(1000))
    tx.weight = manager.daa_factory.minimum_tx_weight(tx)
    tx.update_hash()
    assert manager.propagate_tx(tx)
    simulator.run(60)
    balance_per_address = manager.wallet.get_balance_per_address(settings.HATHOR_TOKEN_UID)
    assert balance_per_address[address] == TokenAmount.from_v1(1000)
...
def simulate_empty_script(simulator: 'Simulator', manager: 'HathorManager') -> Optional['DAGArtifacts']:
    ...
    tx1 = gen_new_tx(manager, address, TokenAmount.from_v1(1000))
    ...
    tx2 = gen_new_tx(manager, address, TokenAmount.from_v1(1000))
    tx2.inputs = [TxInput(tx_id=tx1.hash, index=1, data=b'\x51')]
    tx2.outputs = [TxOutput(value=TokenAmount.from_v1(1000), script=original_script)]
