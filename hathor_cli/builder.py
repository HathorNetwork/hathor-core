...
class CliBuilder:
    ...
    def create_manager(self, reactor: Reactor) -> HathorManager:
        ...
        # Create SandboxAPIConfigLoader if nano contracts are enabled and config file is specified
        if settings.ENABLE_NANO_CONTRACTS and settings.NC_SANDBOX_API_CONFIG_FILE:
            self.sandbox_api_config_loader = SandboxAPIConfigLoader(settings.NC_SANDBOX_API_CONFIG_FILE)
