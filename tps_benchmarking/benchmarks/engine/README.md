...
only the latest is a tip. The alternative `transparent` workload parents every tx to genesis → all N are
tips → consensus (`mempool_tips.update`) is O(tips) = O(N) → the batch costs **O(N²)** (kept on purpose to
...
- **Linear (single-tip) DAG.** `organic` keeps ≈ 1 tip; mainnet runs ~2–3 tips. Enough to remove the
  O(N²) artifact and measure steady cost, but not a perfect mainnet topology.
