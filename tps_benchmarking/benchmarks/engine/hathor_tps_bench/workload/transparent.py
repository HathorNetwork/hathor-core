 ...
 * `DefunctTxSource` (key `defunct`, genesis-parented): we DON'T emit parent edges, so the
   filler gives every tx two genesis parents. No tx is ever a parent of another => EVERY tx is
   a tip => tips = N => consensus is O(N) per tx => O(N^2) for the batch. The pathological
   baseline — kept ONLY to demonstrate the effect (hence "defunct"; not for real measurement).

 * `OneTipTransparentTxSource` (key `1-tip-transparent`, tip-confirming chain): each tx names
   the previous tx as a parent (`tx_k --> tx_{k-1}`), so each tx confirms its predecessor and
   only the latest tx is ever a tip => tips ~= 1 => consensus is O(1) per tx => flat. The
   realistic transparent workload (leaves room for a future `k-tip-transparent`).
...
class TransparentTxSource(TxSource):
    """Shared transparent-output DAG builder — the BASE for the selectable transparent
    workloads below; not registered/selectable on its own. Holds all the funding + render_dsl
    logic and the per-output hooks. Its default `_frontier_lines` is genesis-parenting, which
    is exactly the `defunct` workload's behaviour (DefunctTxSource simply registers it).
    Subclasses override `_frontier_lines` to choose a tip policy. See module docstring."""
    ...
    def render_dsl(self, num_txs: int, num_inputs: int, num_outputs: int) -> str:
        ...
        # --- value bookkeeping -------------------------------------------------------
        # `per` = the value we give each minted UTXO, and hence each tx input. Base case
        # (transparent, fpo=0) it is `num_outputs`, so the per-tx arithmetic below divides
        # cleanly into >=1 per output. Shielded outputs additionally cost `fpo` HTR each in
        # fee (paid from the inputs), so we raise `per` to keep each output >= 1 after the fee:
        #   need  num_inputs*per - num_outputs*fpo >= num_outputs.
        fpo = self._fee_per_output()                       # 0 transparent; FEE_PER_*_SHIELDED for shielded
        suffix = self._output_suffix()                     # "" transparent; " [shielded]"/" [full-shielded]"
        per = max(num_outputs, math.ceil(num_outputs * (1 + fpo) / num_inputs))
        ...
        # Output split: a tx has num_inputs*per HTR in, minus the total shielded fee
        # (num_outputs*fpo, 0 for transparent), to distribute across num_outputs outputs.
        # Give each `base`, and let the LAST output absorb any remainder so the outputs sum
        # EXACTLY to inputs-minus-fee (=> balanced => filler adds nothing => exact I/O).
        base, rem = divmod(num_inputs * per - num_outputs * fpo, num_outputs)
        ...
        for t in range(num_txs):
            ...
            for j in range(num_outputs):
                ...
                lines.append(f"{name}.out[{j}] = {v} HTR{suffix}")  # pin each output (+ shielded attr)
            # The parent/ordering line(s) — the ONLY thing that differs between the
            # genesis-parented (defunct) baseline and the 1-tip-transparent chain. See _frontier_lines.
    ...
    # --- per-output hooks (overridden by shielded sources) ----------------------------
    def _fee_per_output(self) -> int:
        """HTR fee charged per payload output by the protocol. 0 for transparent; shielded
        sources return FEE_PER_AMOUNT/FULL_SHIELDED_OUTPUT so the funding covers it."""
        return 0

    def _output_suffix(self) -> str:
        """DSL attribute appended to each payload output line. '' for transparent; shielded
        sources return ' [shielded]' (amount-only) or ' [full-shielded]'."""
        return ""
    ...
    def build(self, harness: Any, num_txs: int, num_inputs: int, num_outputs: int) -> list[PreparedTx]:
        ...
        for node, vertex in artifacts.list:
            if node.name in targets:
                ...
                # 1-tip-transparent chain) have already been driven and exist in storage.
...
@register_txtype("defunct")
class DefunctTxSource(TransparentTxSource):
    """`defunct` — genesis-parented baseline: we emit NO parent edges, so the filler gives every
    tx two genesis parents. No tx is ever another tx's parent => EVERY tx is a tip => tips = N =>
    the mempool-tips scan is O(N) per tx => O(N^2) for the batch. Pathological; kept ONLY to
    demonstrate the effect (hence "defunct" — not for real measurement). Inherits the base's
    genesis-parenting `_frontier_lines` unchanged."""


@register_txtype("1-tip-transparent")
class OneTipTransparentTxSource(TransparentTxSource):
    """`1-tip-transparent` — tip-confirming workload: each tx names the PREVIOUS tx as a parent, so
    ...
    Funding / inputs / outputs are IDENTICAL to `defunct`; ONLY the parent edges
