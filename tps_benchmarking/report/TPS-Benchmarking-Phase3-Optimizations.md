...
Phase 1 built the instrument and measured the **baseline**: how fast a Hathor full node processes a
transaction on one thread, and where the time goes. Phase 2 added **shielded** (confidential) transactions
as a preliminary addendum. This phase answers the natural follow‑up: **an external optimization PR claims a
multi‑× speed‑up — is it real, where does it come from, and what does it (and does it not) do for shielded
transactions?**
...
**The 1‑tip workload, recapped.** Every Hathor transaction confirms two "parents". If a synthetic workload
lets every transaction parent *genesis*, then no transaction is ever another's parent, so **all N
transactions are tips**; because consensus re‑scans all current tips on every transaction
(`mempool_tips.update` is O(tips)), per‑transaction cost in S5 grows with the batch and the run costs
**O(N²)**. We avoid this by **chaining the transactions in the parent DAG** — `tx_k` names `tx_{k-1}` as a
parent — so each transaction confirms its predecessor and **only the latest is ever a tip**. The tip set
stays ≈ 1, the tip scan is O(1), and S5 is flat in N. This "1‑tip‑transparent" construction (and its
shielded counterpart) is what lets us read a *steady per‑transaction* cost instead of a batch artifact; it
is also, deliberately, *simpler* than mainnet's 2–3‑tip mesh (see §4).
...
**Correctness model.** Every Rust path keeps **Python as the authoritative reference**, guarded three ways:
*fallback* (anything Rust can't handle defers to Python, so all rejection semantics stay Python),
*differential testing* (corpus + mutation + fuzz, asserting identical accept/reject **and error category**),
and *shadow mode* (Python authoritative while Rust runs alongside and mismatches are logged). We merged the
optimizations **behind flags, keeping both implementations in the tree**, and verified the gated‑on path is
**behaviour‑identical** to the baseline (stored state, mempool tips, and serialized vertex bytes match
byte‑for‑byte with all sections on vs off).
...
**Conclusion — inputs cost signatures.** Each additional input is one more ECDSA signature to verify, so the
decline lives almost entirely in **`verify` (S3S4)**, which roughly doubles from 1 to 8 inputs; `cons.` and
`post‑cons.` are nearly flat. This is the transparent world's defining cost axis — and precisely the work the
PR's **parallel Rust script pool** targets, which is why the drop is gentle rather than steep (the per‑input
ECDSA checks fan across up to four worker threads).
...
**Conclusion — the gap widens with size.** Transparent cost grows slowly (signatures fan across cores);
shielded cost grows with **output count** on a **serial** verifier, so the ratio blows out from 10× at 1i2o
to **38× at 8i8o**. An 8‑output shielded transaction verifies **≈ 77 ms** of range proofs on one thread. The
divergence is not a constant "privacy tax" — it is a scaling penalty concentrated on the un‑parallelized
...
**Conclusion — throughput tracks outputs, almost independently of inputs.** Reading down a column (fixed
...
*Phase 3 — the optimization PR, measured for transparent and shielded transactions behind runtime flags.
Generated from the `hathor_tps_bench` benchmark engine; per‑stage timings, plots (PNG + vector PDF), and raw
per‑cell CSV/JSON are reproducible via `report_data.py` / `report_plots.py`. Data: N = 2000, k = 3, median,
i5‑11300H single thread.*
