...
- **`hathor-next` is an experimental crate** — a partial Rust port of `hathor-core` (p2p + vertex serialization). It is a normal workspace member (shares the root `Cargo.lock`, inherits workspace lints) and *is* covered by `just all` / CI, including the `--all-features` recipes. Its `tokio-console` feature does **not** `compile_error!` when built without `--cfg tokio_unstable`; instead the console layer is skipped at runtime (it would receive no task data) and a warning is emitted — see `crates/hathor-next/src/logging.rs::init_subscriber`. This is what keeps `--all-features` compiling. The crate also ships its own `cargo dev-*` aliases (`dev-check`, `dev-test`, `dev-clippy`, `dev-run`) in `crates/hathor-next/.cargo/config.toml` for building against all three crypto backends at once; that config is intentionally crate-local so its `panic = "abort"` and `target-cpu=native` settings don't leak into the PyO3 cdylib crates, which are built from the workspace root.
