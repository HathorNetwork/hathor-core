...
- **64-bit only.** `crates/htr-lib-py/src/lib.rs` emits `compile_error!` on non-64-bit targets so `usize` semantics stay consistent across platforms. Code may assume `usize` is 64 bits.
