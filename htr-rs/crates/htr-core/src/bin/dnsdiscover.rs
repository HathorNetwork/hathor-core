...
use htr_core::{discovery, logging, utils};
use thiserror::Error;
use tracing::*;
...
fn main() -> Result<(), CliError> {
    ...
    let domain = if let Some(d) = &cli.domain {
        ...
        htr_core::network_info::NETWORK_INFO_MAINNET
            .bootstrap_txt_domain
            .as_deref()
            .unwrap()
            .to_string()
    } else if cli.testnet_golf {
        htr_core::network_info::NETWORK_INFO_TESTNET_GOLF
            .bootstrap_txt_domain
            .as_deref()
            .unwrap()
            .to_string()
    } else if cli.testnet_hotel {
        htr_core::network_info::NETWORK_INFO_TESTNET_HOTEL
            .bootstrap_txt_domain
            .as_deref()
            .unwrap()
            .to_string()
    } else if cli.testnet_india || cli.testnet {
        htr_core::network_info::NETWORK_INFO_TESTNET_INDIA
            .bootstrap_txt_domain
            .as_deref()
            .unwrap()
            .to_string()
