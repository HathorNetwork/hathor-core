...
use htr_core::vertex::DagData;
use htr_core::vertex::{AnyVertexData, decode_any_vertex_data, encode_any_vertex_data};
use thiserror::Error;
use tracing::*;
...
fn main() -> Result<(), RtError> {
    ...
    let logs_dir = htr_core::utils::project_dir()
        .unwrap_or_else(|| std::env::current_dir().expect("cwd"))
        .join("logs");
    std::fs::create_dir_all(&logs_dir)?;
    let _guard = htr_core::logging::setup_logging_with_level(&logs_dir, fallback_level)
