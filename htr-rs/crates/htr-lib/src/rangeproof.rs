...
use secp256k1_zkp::{Generator, PedersenCommitment, RangeProof, SECP256K1, SecretKey, Tweak};
...
mod tests {
    ...
    fn test_valid_range_proof() {
        ...
        let amount = 1000u64;
        let commitment = create_commitment(amount, &blinding, &genr).unwrap();

        let proof = create_range_proof(amount, &blinding, &commitment, &genr, None, None).unwrap();
        assert!(verify_range_proof(&proof, &commitment, &genr).is_ok());
    }
    ...
    fn test_zero_amount_rejected() {
        ...
        let result = create_range_proof(amount, &blinding, &commitment, &genr, None, None);
        assert!(
            result.is_err(),
    ...
    fn test_wrong_commitment_fails() {
        ...
        let commitment1 = create_commitment(1000, &blinding1, &genr).unwrap();
        let commitment2 = create_commitment(2000, &blinding2, &genr).unwrap();

        let proof = create_range_proof(1000, &blinding1, &commitment1, &genr, None, None).unwrap();
        // Verify with wrong commitment should fail
        assert!(verify_range_proof(&proof, &commitment2, &genr).is_err());
    }
    ...
    fn test_batch_verify() {
        ...
        let generators = vec![genr; 3];

        for amount in amounts {
            let blinding = Tweak::new(&mut rand::thread_rng());
            let commitment = create_commitment(amount, &blinding, &genr).unwrap();
            let proof =
                create_range_proof(amount, &blinding, &commitment, &genr, None, None).unwrap();
            proofs.push(proof);
    ...
    fn test_serialization_roundtrip() {
        let genr = htr_asset_tag();
        let blinding = Tweak::new(&mut rand::thread_rng());
        let commitment = create_commitment(500, &blinding, &genr).unwrap();
        let proof = create_range_proof(500, &blinding, &commitment, &genr, None, None).unwrap();

        let bytes = serialize_range_proof(&proof);
        let proof2 = deserialize_range_proof(&bytes).unwrap();
        assert!(verify_range_proof(&proof2, &commitment, &genr).is_ok());
    }
    ...
    fn test_proof_with_message() {
        ...
        let proof =
            create_range_proof(amount, &blinding, &commitment, &genr, Some(msg), None).unwrap();
        assert!(verify_range_proof(&proof, &commitment, &genr).is_ok());
    }
    ...
    fn test_rewind_roundtrip() {
        ...
        let (recovered_value, recovered_blinding, recovered_message) =
            rewind_range_proof(&proof, &commitment, &nonce, &genr).unwrap();
    ...
    fn test_rewind_wrong_nonce_fails() {
        ...
        let proof =
            create_range_proof(amount, &blinding, &commitment, &genr, None, Some(&nonce)).unwrap();

        // Rewind with wrong nonce should fail
        let result = rewind_range_proof(&proof, &commitment, &wrong_nonce, &genr);
