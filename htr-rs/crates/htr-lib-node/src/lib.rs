...
fn parse_generator(bytes: &[u8]) -> napi::Result<Generator> {
    ...
    htr_lib::generators::deserialize_generator(bytes).map_err(to_napi_err)
}
...
pub fn derive_asset_tag(token_uid: Buffer) -> napi::Result<Buffer> {
    ...
    let tag = htr_lib::generators::derive_asset_tag(&uid).map_err(to_napi_err)?;
...
pub fn derive_tag(token_uid: Buffer) -> napi::Result<Buffer> {
    ...
    let uid: [u8; 32] = token_uid
        ...
        .try_into()
        .map_err(|_| napi::Error::from_reason("token_uid must be exactly 32 bytes"))?;
    let tag = htr_lib::generators::derive_tag(&uid).map_err(to_napi_err)?;
...
pub fn create_asset_commitment(tag_bytes: Buffer, r_asset: Buffer) -> napi::Result<Buffer> {
    ...
    let commitment =
        htr_lib::generators::create_asset_commitment(&tag, &tweak).map_err(to_napi_err)?;
...
pub fn create_commitment(
    ...
    let c = htr_lib::pedersen::create_commitment(amount, &bf, &gen).map_err(to_napi_err)?;
...
pub fn create_trivial_commitment(amount: BigInt, generator: Buffer) -> napi::Result<Buffer> {
    let amount = bigint_to_u64(&amount)?;
    let gen = parse_generator(generator.as_ref())?;
    let c = htr_lib::pedersen::create_trivial_commitment(amount, &gen).map_err(to_napi_err)?;
    Ok(Buffer::from(c.serialize().to_vec()))
}
...
pub fn verify_commitments_sum(positive: Vec<Buffer>, negative: Vec<Buffer>) -> napi::Result<bool> {
    let pos: Vec<_> = positive
        .iter()
        .map(|b| htr_lib::pedersen::deserialize_commitment(b.as_ref()).map_err(to_napi_err))
...
pub fn create_range_proof(
    ...
    let bf = parse_tweak(blinding.as_ref())?;
    let comm =
        htr_lib::pedersen::deserialize_commitment(commitment.as_ref()).map_err(to_napi_err)?;
    let gen = parse_generator(generator.as_ref())?;
    let nonce_key = nonce
        .as_ref()
...
pub fn verify_range_proof(
    ...
    let p = htr_lib::rangeproof::deserialize_range_proof(proof.as_ref()).map_err(to_napi_err)?;
    let c = htr_lib::pedersen::deserialize_commitment(commitment.as_ref()).map_err(to_napi_err)?;
    let gen = parse_generator(generator.as_ref())?;
    match htr_lib::rangeproof::verify_range_proof(&p, &c, &gen) {
        Ok(range) => {
            if range.start < 1 {
...
pub fn rewind_range_proof(
    ...
    let (value, blinding, message) =
        htr_lib::rangeproof::rewind_range_proof(&p, &c, &sk, &gen).map_err(to_napi_err)?;
    Ok(RewindResult {
...
pub fn validate_commitment(data: Buffer) -> bool {
    ...
    htr_lib::pedersen::deserialize_commitment(data.as_ref()).is_ok()
}
...
pub fn validate_generator(data: Buffer) -> bool {
    ...
    htr_lib::generators::deserialize_generator(data.as_ref()).is_ok()
}
...
pub fn create_surjection_proof(
    ...
    let proof = htr_lib::surjection::create_surjection_proof(&ct, &cbf, &domain_vec)
        .map_err(to_napi_err)?;
    Ok(Buffer::from(proof.serialize().to_vec()))
}
...
pub fn verify_surjection_proof(
    ...
    let p =
        htr_lib::surjection::deserialize_surjection_proof(proof.as_ref()).map_err(to_napi_err)?;
...
pub fn verify_balance(
    ...
    for cb in &shielded_inputs {
        let c = htr_lib::pedersen::deserialize_commitment(cb.as_ref()).map_err(to_napi_err)?;
...
pub fn generate_random_blinding_factor() -> Buffer {
    Buffer::from(htr_lib::ecdh::generate_random_blinding_factor().to_vec())
}
...
pub fn derive_ecdh_shared_secret(private_key: Buffer, peer_pubkey: Buffer) -> napi::Result<Buffer> {
    let sk = htr_lib::ecdh::parse_secret_key(private_key.as_ref()).map_err(to_napi_err)?;
    let pk = htr_lib::ecdh::parse_public_key(peer_pubkey.as_ref()).map_err(to_napi_err)?;
...
pub fn create_shielded_output_with_both_blindings(
    ...
    let result = htr_lib::ecdh::create_full_shielded_output(
        value,
...
pub fn create_amount_shielded_output(
    ...
    let result =
        htr_lib::ecdh::create_amount_shielded_output(value, recipient_pubkey.as_ref(), &tuid, &vbf)
            .map_err(to_napi_err)?;
...
pub fn get_generator_size() -> u32 {
    htr_lib::types::GENERATOR_SIZE as u32
}
