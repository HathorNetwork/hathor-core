...
#[command(
    name = "peer-crawl",
    version,
    about = "Bootstrap peers via DNS, crawl the graph with GET-PEERS, and report discovered nodes"
)]
struct Cli {
...
fn main() -> Result<(), CliError> {
    ...
    for (peer_id, node) in &report.peers {
        println!("peer {}", peer_id);
        if let Some(hello) = &node.hello {
            println!("  app: {}", hello.app);
            println!("  capabilities: {:?}", hello.capabilities);
        }
        if let Some(pp) = &node.public_peer
            && !pp.endpoints.is_empty()
        {
            println!("  advertised entrypoints:");
            for ep in &pp.endpoints {
                println!("    {}", ep);
            }
        }
        if !node.observed_endpoints.is_empty() {
            println!("  observed endpoints:");
            for ep in &node.observed_endpoints {
                println!("    {}", ep);
            }
        }
        if !node.inbound_peers.is_empty() {
            println!(
                "  inbound connections from {} peer(s):",
                node.inbound_peers.len()
            );
            for inbound in &node.inbound_peers {
                println!("    {}", inbound);
            }
        }
        if !node.relayed_endpoints.is_empty() {
            println!("  relayed endpoints:");
            for ep in &node.relayed_endpoints {
                println!("    {}", ep);
            }
        }
        if !node.advertises.is_empty() {
            println!("  advertises {} peer(s)", node.advertises.len());
        }
        if !node.advertised_by.is_empty() {
            println!("  advertised by {} peer(s)", node.advertised_by.len());
        }
        if node.peers_timeout {
            println!("  note: GET-PEERS timed out before completion");
        }
    }
...
struct JsonPeer {
    peer_id: peer::PeerId,
    hello: Option<hathor_next::protocol::message::HelloData>,
    public_peer: Option<peer::PublicPeer>,
    advertises: Vec<peer::PeerId>,
    advertised_by: Vec<peer::PeerId>,
    observed_endpoints: Vec<peer::PeerEndpoint>,
    inbound_peers: Vec<peer::PeerId>,
    relayed_endpoints: Vec<peer::PeerEndpoint>,
    peers_timeout: bool,
}
...
impl JsonReport {
    fn from_report(
        ...
        let peers = report
            .peers
            .iter()
            .map(|(peer_id, node)| JsonPeer {
                peer_id: *peer_id,
                hello: node.hello.clone(),
                public_peer: node.public_peer.clone(),
                advertises: node.advertises.iter().copied().collect(),
                advertised_by: node.advertised_by.iter().copied().collect(),
                observed_endpoints: node.observed_endpoints.iter().cloned().collect(),
                inbound_peers: node.inbound_peers.iter().copied().collect(),
                relayed_endpoints: node.relayed_endpoints.iter().cloned().collect(),
                peers_timeout: node.peers_timeout,
            })
            .collect();
