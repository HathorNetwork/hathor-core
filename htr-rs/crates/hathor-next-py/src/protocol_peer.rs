//! A sans-IO stepper over `hathor-next`'s real protocol state machine, exposed to Python.
//!
//! `ProtocolPeer` wraps the type-state `Engine` from `hathor_next::protocol::state` and drives it
...
use hathor_next::peer::{PrivatePeer, PublicPeer};
use hathor_next::protocol::ProtocolDriver;
use hathor_next::protocol::message::{
    ControlMessage, HelloData, HelloStateMessage, PeerIdMessage, PeerIdStateMessage,
    ReadyStateMessage,
};
use hathor_next::protocol::state::{
...
/// async I/O engine in `hathor_next::protocol::engine`.
...
/// A scriptable peer that drives the real `hathor-next` protocol state machine from Python.
#[pyclass]
