...
This guide explains how wallets and explorers interact with Hathor's shielded outputs using the `htr-lib` library. All three bindings -- Rust (direct), Python (PyO3), and Node.js/TypeScript (NAPI) -- wrap the same Rust implementation.

> **Scope:** This guide covers creating, verifying, recovering, and spending shielded outputs. Transaction assembly (attaching headers, signing, etc.) is covered in the transaction format specification; this guide focuses on the `htr-lib` primitives the wallet and explorer need.
...
```rust
use htr_lib::ecdh::generate_random_blinding_factor;
...
```python
from htr_lib import shielded as ct
...
```typescript
import { generateRandomBlindingFactor } from 'htr-lib-node';
...
```rust
use htr_lib::ecdh::{create_amount_shielded_output, generate_random_blinding_factor};
...
```rust
use htr_lib::ecdh::{create_full_shielded_output, generate_random_blinding_factor};
...
```typescript
import { createShieldedOutputWithBothBlindings } from 'htr-lib-node';
...
```rust
use htr_lib::balance::compute_balancing_blinding_factor;
...
```typescript
import { computeBalancingBlindingFactor } from 'htr-lib-node';
...
use htr_lib::rangeproof::verify_range_proof;
...
```rust
// Rust
use htr_lib::surjection::verify_surjection_proof;
...
```rust
use htr_lib::balance::{BalanceEntry, verify_balance};
...
```rust
use htr_lib::ecdh::rewind_amount_shielded_output;
...
```typescript
import { rewindAmountShieldedOutput } from 'htr-lib-node';
...
```rust
use htr_lib::ecdh::rewind_full_shielded_output;
...
```typescript
import { rewindFullShieldedOutput } from 'htr-lib-node';
...
```rust
use htr_lib::generators::{derive_tag, create_asset_commitment};
...
```typescript
import { deriveTag, createAssetCommitment } from 'htr-lib-node';
