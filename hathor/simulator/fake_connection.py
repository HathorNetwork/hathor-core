...
class FakeConnection:
    ...
    def is_both_synced(self, *, errmsgs: Optional[list[str]] = None) -> bool:
        ...
        if not state1_is_synced or not state2_is_synced:
            ...
            return False
        [best_block_info1] = state1.dependencies.get_n_height_tips(1)
        [best_block_info2] = state2.dependencies.get_n_height_tips(1)
        if best_block_info1.id != best_block_info2.id:
            self.log.debug('best block is different')
            errmsgs.append('best block is different')
            return False
        tips1 = {i.data for i in state1.dependencies.get_tx_tips()}
        tips2 = {i.data for i in state2.dependencies.get_tx_tips()}
        if tips1 != tips2:
    ...
    def reconnect(self) -> None:
        ...
        self._buf1.clear()
        self._buf2.clear()
        self._proto1 = self.manager1.p2p_manager.get_server_factory().buildProtocol(HostnameAddress(b'fake', 0))
        self._proto2 = self.manager2.p2p_manager.get_client_factory().buildProtocol(HostnameAddress(b'fake', 0))
