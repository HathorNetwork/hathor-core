...
class FeatureErrorResponse(ErrorResponseModel):
    """Error response for feature endpoint. Preserves HTTP 200 for backwards compatibility."""
...
class FeatureResource(Resource):
    ...
    @api_endpoint(
        ...
        response_model=Union[GetFeaturesResponse, GetBlockFeaturesResponse, FeatureErrorResponse],
    )
    ...
    def _get_block_features(self, params: GetBlockFeaturesParams) -> ResponseModel:
        ...
        if not isinstance(block, Block):
            return FeatureErrorResponse(error=f"Hash '{params.block.hex()}' is not a Block.")
