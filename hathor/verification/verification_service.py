...
class VerificationService:
    __slots__ = ('_reactor', '_settings', 'verifiers', '_tx_storage', '_daa', '_multiprocessor')
    ...
    def __init__(
        ...
        *,
        ...
        multiprocessor: Multiprocessor | None,
    ) -> None:
...
def _verify_basic_model(
    ...
    *,
    ...
    verifiers.vertex.verify_version(verification_model.vertex)
...
def _verify_basic_poa_block(verifiers: VertexVerifiers, block: PoaBlock) -> None:
    verifiers.poa_block.verify_poa(block)
    # TODO
    raise NotImplementedError
    # verifiers.block.verify_reward(block)
    ...
    *,
    ...
    match verification_model:
        ...
        case VerificationTokenCreationTransaction(vertex=tx, deps=deps):
            _verify_token_creation_tx(verifiers, tx, not_none(deps), reject_locked_reward=reject_locked_reward)
        case _:
            assert_never(verification_model)
...
def verify_without_storage(verifiers: VertexVerifiers, vertex: Vertex) -> None:
...
def _verify_without_storage_base_block(verifiers: VertexVerifiers, block: Block) -> None:
...
def _verify_without_storage_block(verifiers: VertexVerifiers, block: Block) -> None:
    ...
    _verify_without_storage_base_block(verifiers, block)
...
def _verify_without_storage_poa_block(verifiers: VertexVerifiers, block: PoaBlock) -> None:
    _verify_without_storage_base_block(verifiers, block)
