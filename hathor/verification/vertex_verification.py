...
def verify_parents(vertex: BaseTransaction, *, settings: HathorSettings) -> None:
        ...
        try:
            ...
            if parent.is_block:
                if vertex.is_block and not parent.is_genesis:
                    if vertex.timestamp - parent.timestamp > settings.MAX_DISTANCE_BETWEEN_BLOCKS:
...
def verify_pow(vertex: BaseTransaction, override_weight: Optional[float] = None) -> None:
...
def verify_outputs(vertex: BaseTransaction, *, settings: HathorSettings) -> None:
...
def verify_sigops_output(vertex: BaseTransaction, *, settings: HathorSettings) -> None:
