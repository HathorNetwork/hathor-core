...
class VertexVerifier:
    ...
    # CONS-025: Map header types to their VertexHeaderId for canonical ordering
    _HEADER_ORDER: ClassVar[dict[type, int]] = {}

    @classmethod
    def _get_header_order(cls, header: VertexBaseHeader) -> int:
        """Return the sort key for canonical header ordering."""
        if not cls._HEADER_ORDER:
            from hathor.transaction.headers import FeeHeader, NanoHeader, ShieldedOutputsHeader, VertexHeaderId
            cls._HEADER_ORDER = {
                NanoHeader: int.from_bytes(VertexHeaderId.NANO_HEADER.value, 'big'),
                FeeHeader: int.from_bytes(VertexHeaderId.FEE_HEADER.value, 'big'),
                ShieldedOutputsHeader: int.from_bytes(VertexHeaderId.SHIELDED_OUTPUTS_HEADER.value, 'big'),
            }
        return cls._HEADER_ORDER.get(type(header), 0)
