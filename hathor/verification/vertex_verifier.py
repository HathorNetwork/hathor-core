...
from hathor.verification.verification_model import VertexDependencies
...
class VertexVerifier:
    ...
    def verify_parents(self, vertex: BaseTransaction, deps: VertexDependencies) -> None:
        ...
        for parent_hash in vertex.parents:
            try:
                parent = deps.storage.get_vertex(parent_hash)
                ...
                if parent.is_block:
                    ...
                    for pi_hash in parent.parents:
                        pi = deps.storage.get_vertex(parent_hash)
                        if not pi.is_block:
                            min_timestamp = (
