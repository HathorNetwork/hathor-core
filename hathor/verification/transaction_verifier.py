...
class TransactionVerifier:
    ...
    def verify_reward_locked(self, tx: Transaction, tx_deps: TransactionDependencies) -> None:
        ...
        self.verify_reward_locked_for_height(tx, tx_deps.reward_locked_info, tx_deps.best_block_height)
    ...
    def verify_reward_locked_for_height(
        ...
        *,
        ...
        Args:
            ...
            reward_locked_info: the reward locked info obtained from `get_spent_reward_locked_info()`
        ...
        if best_height + 1 < tx.static_metadata.min_height:
            if assert_min_height_verification:
                raise AssertionError('a new tx should never be invalid by its inherited min_height.')
            raise RewardLocked(
                f'Tx {tx.hash_hex} has min_height={tx.static_metadata.min_height}, but the best_height={best_height}.'
            )
