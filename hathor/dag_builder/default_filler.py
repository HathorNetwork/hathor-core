...
class DefaultFiller:
    ...
    def find_txin(self, amount: TokenAmount, token: str) -> DAGInput:
    ...
    def calculate_balance(self, node: DAGNode) -> dict[str, TokenBalance]:
        """Calculate the balance for each token in a node.

        balance = sum(outputs) - sum(inputs)
        """
        ins: defaultdict[str, TokenAmount] = defaultdict(TokenAmount.zero)
        ...
        outs: defaultdict[str, TokenAmount] = defaultdict(TokenAmount.zero)
        ...
        balance = {}
        for key in keys:
            balance[key] = outs[key].to_balance() - ins[key].to_balance()
    ...
    def balance_node_inputs_and_outputs(self, node: DAGNode) -> None:
        ...
        for key, diff in balance.items():
            target = node.balances.get(key, TokenBalance(0))
            diff -= target
            if diff < TokenBalance(0):
                index = self.get_next_index(node.outputs)
                node.outputs[index] = DAGOutput((-diff).to_amount(), key, {'_origin': 'f3'})
            elif diff > TokenBalance(0):
                txin = self.find_txin(diff.to_amount(), key)
    ...
    def run(self) -> None:
        ...
        for node in self._builder._nodes.values():
            ...
            for i in range(len(node.outputs)):
                ...
                elif txout.amount == TokenAmount.zero():
        ...
        for node in list(self._builder.topological_sorting()):
            match node.type:
                ...
                case DAGNodeType.Block:
                    ...
                    diff = balance.get('HTR', TokenBalance(0))

                    # TODO Use the actual height. DAG construction has no chain context, so V1.
                    target = self._daa_factory.create_v1().get_tokens_issued_per_block(1).to_balance()
                    assert diff >= TokenBalance(0)
                    assert diff <= target

                    if diff < target:
                        node.outputs.append(DAGOutput((target - diff).to_amount(), 'HTR', {'_origin': 'f7'}))
        ...
        for token in tokens:
            ...
            htr_deposit: TokenAmount

            match token_version:
                case TokenVersion.NATIVE:
                    raise AssertionError
                case TokenVersion.DEPOSIT:
                    htr_deposit = get_deposit_token_deposit_amount(self._settings, balance[token].to_amount())
                case TokenVersion.FEE:
                    htr_deposit = TokenAmount.zero()

            htr_balance = balance.get('HTR', TokenBalance(0))

            # target = sum(outputs) - sum(inputs)
            # <0 means deposit
            # >0 means withdrawal
            htr_target = node.balances.get('HTR', TokenBalance(0)) - htr_deposit.to_balance()

            diff = htr_balance - htr_target

            if diff < TokenBalance(0):
                index = self.get_next_index(node.outputs)
                node.outputs[index] = DAGOutput((-diff).to_amount(), 'HTR', {'_origin': 'f8'})

            elif diff > TokenBalance(0):
                txin = self.find_txin(diff.to_amount(), 'HTR')
                node.inputs.add(txin)
        ...
        if 'dummy' in self._builder._nodes:
            ...
            if not balance:
                ...
                if diff < TokenBalance(0):
                    index = self.get_next_index(node.outputs)
                    node.outputs[index] = DAGOutput((-diff).to_amount(), 'HTR', {})
