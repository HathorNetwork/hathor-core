...
class DAGBuilder:
    ...
    def update_balance(self, name: str, token: str, value: TokenBalance) -> Self:
        ...
        node.balances[token] = node.balances.get(token, TokenBalance(0)) + value
        if token != 'HTR':
    ...
    def add_spending_edge(self, _from: str, _to: str, _txout_index: int) -> Self:
        ...
        if len(to_node.outputs) <= _txout_index:
            ...
            to_node.outputs[_txout_index] = DAGOutput(TokenAmount.zero(), '', {})
        from_node = self._get_or_create_node(_from)
    ...
    def _add_nc_attribute(self, name: str, key: str, value: str) -> None:
        ...
        if key == 'nc_id':
            ...
            token_amount = node.as_node_amount(amount)
            token_balance = token_amount.to_balance()
    ...
    def _append_fee(self, name: str, key: str, value: str) -> None:
        ...
        token_amount = node.as_node_amount(amount)
        self.update_balance(name, token, -token_amount.to_balance())
    ...
    def add_attribute(self, name: str, key: str, value: str) -> Self:
        ...
        if key.startswith('balance_'):
            ...
            self.update_balance(name, token, node.as_node_amount(int(value)).to_balance())
            return self
