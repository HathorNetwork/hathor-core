...
"""
This module contains a custom Result type for explicit error handling,
inspired by Rust and adapted from https://github.com/rustedpy/result.
"""
...
class Ok(Generic[T]):
    ...
    def unwrap_or_raise_another(self, _e: object) -> T:
        """
        Return the value.
        """
        return self._value
...
class Err(Generic[E]):
    ...
    __slots__ = ('_value', 'cause', 'traceback')
    __match_args__ = ('_value',)

    def __init__(self, value: E, cause: Exception | None = None) -> None:
        """
        Create an Err() instance. When called in the except block of a try-except, provide the caught exception as
        the `cause` argument to include its traceback in the Err. Otherwise, if the `value` argument itself is an
        Exception, we'll automatically retrieve the traceback.
        """
        self._value = value
    ...
    def _capture_traceback(e: Exception) -> str:
        ...
        ATTENTION: Must only be called from `Err.__init__`.
        """
        ...
        # Filter stack to start from the first frame in either /hathor or /tests
        filtered_stack: deque[traceback.FrameSummary] = deque()

        # Consume iterator in reverse until we find the first irrelevant frame
    ...
    def unwrap_or_raise(self) -> NoReturn:
        """
        The contained result is `Err`, and the value is an exception, so raise itself.
        """
    ...
    def unwrap_or_raise_another(self, e: Type[TBE]) -> NoReturn:
        """
    ...
    def unwrap_or_propagate(self) -> NoReturn:
        """
        The contained result is `Err`, so raise the internal _ResultPropagationException with self.

        This method is used to emulate Rust's question mark operator. It must be used together with the
        `@propagate_result` annotation on the function that is calling this method.
        """
...
class _ResultPropagationException(Exception):
    """Helper exception to emulate Rust's question mark operator. For internal use only."""
    def __init__(self, err: Err[E]) -> None:
        # the message below will appear in the call trace when the user forgets the decorator.
        super().__init__('did you forget to decorate the function/method with `@propagate_result`?')
        self.err = err


def propagate_result(f: Callable[P, Result[T, E]]) -> Callable[P, Result[T, E]]:
    """
    Decorator to turn a function into one that allows using `unwrap_or_propagate()`.
    It emulates Rust's question mark operator.
    """
    @functools.wraps(f)
