#  Copyright 2025 Hathor Labs
#
#  Licensed under the Apache License, Version 2.0 (the "License");
#  you may not use this file except in compliance with the License.
#  You may obtain a copy of the License at
#
#  http://www.apache.org/licenses/LICENSE-2.0
#
#  Unless required by applicable law or agreed to in writing, software
#  distributed under the License is distributed on an "AS IS" BASIS,
#  WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
#  See the License for the specific language governing permissions and
#  limitations under the License.

"""
This module implements LEB128 for signed integers.

LEB128 or Little Endian Base 128 is a variable-length code compression used to store arbitrarily large
integers in a small number of bytes. LEB128 is used in the DWARF debug file format and the WebAssembly
binary encoding for all integer literals.

References:
- https://en.wikipedia.org/wiki/LEB128
- https://dwarfstd.org/doc/DWARF5.pdf
- https://webassembly.github.io/spec/core/binary/values.html#integers
"""

from hathor.serialization import BytesDeserializer, BytesSerializer, Deserializer, Serializer


def serialize_leb128(value: int, serializer: Serializer, *, signed: bool, max_bytes: int | None = None) -> None:
    """
    Receive a signed integer and push its LEB128-encoded bytes to the end of the given buffer.

    >>> serializer = BytesSerializer()
    >>> serializer.write_bytes(b'test')
    >>> serialize_leb128(0, serializer, signed=True)
    >>> serialize_leb128(624485, serializer, signed=True)
    >>> serialize_leb128(-123456, serializer, signed=True)
    >>> list(serializer.finalize())
    [116, 101, 115, 116, 0, 229, 142, 38, 192, 187, 120]
    """
    err_msg = f'cannot encode more than {max_bytes} bytes'
    cur_size = 0
    if not signed and value < 0:
        raise ValueError('cannot encode value <0 as unsigend')
    while True:
        byte = value & 0b0111_1111
        value >>= 7
        if signed:
            cont = (value == 0 and (byte & 0b0100_0000) == 0) or (value == -1 and (byte & 0b0100_0000) != 0)
        else:
            cont = (value == 0 and (byte & 0b1000_0000) == 0)
        if cont:
            cur_size += 1
            serializer.write_byte(byte)
            if max_bytes is not None and cur_size > max_bytes:
                raise ValueError(err_msg)
            break
        cur_size += 1
        serializer.write_byte(byte | 0b1000_0000)
        if max_bytes is not None and cur_size > max_bytes:
            raise ValueError(err_msg)


def deserialize_leb128(deserializer: Deserializer, *, signed: bool, max_bytes: int | None = None) -> int:
    """ Receive and consume bytes from buffer returning the value.

    A LEB128 with 7 bit group is used.

    >>> data = bytes([0x00, 0xE5, 0x8E, 0x26, 0xC0, 0xBB, 0x78, 0x74, 0x65, 0x73, 0x74])
    >>> deserializer = BytesDeserializer(data)
    >>> deserialize_leb128(deserializer, signed=True)
    0
    >>> deserialize_leb128(deserializer, signed=True)
    624485
    >>> deserialize_leb128(deserializer, signed=True)
    -123456
    >>> bytes(deserializer.read_all())
    b'test'
    """
    result = 0
    shift = 0
    while True:
        byte = deserializer.read_byte()
        result |= (byte & 0b0111_1111) << shift
        shift += 7
        assert shift % 7 == 0
        if max_bytes is not None and shift // 7 > max_bytes:
            raise ValueError(f'cannot decode more than {max_bytes} bytes')
        if (byte & 0b1000_0000) == 0:
            if signed and (byte & 0b0100_0000) != 0:
                return result | -(1 << shift)
            return result


def encode_signed(value: int, *, max_bytes: int | None = None) -> bytes:
    """
    Receive a signed integer and return its LEB128-encoded bytes.

    >>> encode_signed(0) == bytes([0x00])
    True
    >>> encode_signed(624485) == bytes([0xE5, 0x8E, 0x26])
    True
    >>> encode_signed(-123456) == bytes([0xC0, 0xBB, 0x78])
    True
    """
    serializer = BytesSerializer()
    serialize_leb128(value, serializer, max_bytes=max_bytes, signed=True)
    return bytes(serializer.finalize())


def encode_unsigned(value: int, *, max_bytes: int | None = None) -> bytes:
    """
    Receive an unsigned integer and return its LEB128-encoded bytes.

    >>> encode_unsigned(0) == bytes([0x00])
    True
    >>> encode_unsigned(624485) == bytes([0xE5, 0x8E, 0x26])
    True
    """
    serializer = BytesSerializer()
    serialize_leb128(value, serializer, max_bytes=max_bytes, signed=False)
    return bytes(serializer.finalize())


def decode_signed(data: bytes, *, max_bytes: int | None = None) -> tuple[int, bytes]:
    """
    Receive and consume a buffer returning a tuple of the unpacked
    LEB128-encoded signed integer and the reamining buffer.

    >>> decode_signed(bytes([0x00]) + b'test')
    (0, b'test')
    >>> decode_signed(bytes([0xE5, 0x8E, 0x26]) + b'test')
    (624485, b'test')
    >>> decode_signed(bytes([0xC0, 0xBB, 0x78]) + b'test')
    (-123456, b'test')
    """
    deserializer = BytesDeserializer(data)
    value = deserialize_leb128(deserializer, max_bytes=max_bytes, signed=True)
    remaining_data = bytes(deserializer.read_all())
    return (value, remaining_data)


def decode_unsigned(data: bytes, *, max_bytes: int | None = None) -> tuple[int, bytes]:
    """
    Receive and consume a buffer returning a tuple of the unpacked
    LEB128-encoded unsigned integer and the reamining buffer.

    >>> decode_unsigned(bytes([0x00]) + b'test')
    (0, b'test')
    >>> decode_unsigned(bytes([0xE5, 0x8E, 0x26]) + b'test')
    (624485, b'test')
    """
    deserializer = BytesDeserializer(data)
    value = deserialize_leb128(deserializer, max_bytes=max_bytes, signed=False)
    remaining_data = bytes(deserializer.read_all())
    return (value, remaining_data)
