...
from hathor.nanocontracts.consensus import NCBlockRunner
...
class BlockConsensusAlgorithm:
    ...
    def __init__(
        ...
        *,
        ...
        self.nc_block_runner = NCBlockRunner(
            settings=settings,
            context=context,
    ...
    def update_consensus(self, block: Block) -> None:
        ...
        if self._should_execute_nano(block):
            self.nc_block_runner.execute_nano_contracts(block)
        else:
            self.nc_block_runner.nc_initialize_empty(block)
...
class BlockConsensusAlgorithmFactory:
    ...
    def __call__(self, context: 'ConsensusAlgorithmContext') -> BlockConsensusAlgorithm:
        return BlockConsensusAlgorithm(
            ...
            nc_exec_fail_trace=self.nc_exec_fail_trace,
        )
