...
class BlockConsensusAlgorithm:
    ...
    def _nc_execute_calls(self, block: Block, *, is_reorg: bool) -> None:
        ...
        for tx in nc_sorted_calls:
            ...
            from hathor.nanocontracts.runner import Runner  # noqa: F402
            ...
            try:
                ...
                runner.commit()
                self.nc_update_metadata_and_indexes(tx, runner)
            finally:
    ...
    def nc_update_metadata_and_indexes(self, tx: Transaction, runner: 'Runner') -> None:
        from hathor.nanocontracts.runner.types import CallType

        assert tx.storage is not None
        assert tx.storage.indexes is not None
        indexes = tx.storage.indexes
        tx_meta = tx.get_metadata()

        from hathor.transaction.types import MetaNCCallRecord
        assert tx_meta.nc_execution is NCExecutionState.SUCCESS
        ...
        nc_calls: list[MetaNCCallRecord] = [
            MetaNCCallRecord(nc_call.nanocontract_id, nc_call.method_name)
            for nc_call in call_info.calls
            if nc_call.type == CallType.PUBLIC
        ]
        nc_calls_set = set(nc_calls)

        # Add to indexes.
        for contract_id, method_name in nc_calls_set:
            if indexes.nc_history is not None:
                indexes.nc_history.add_single_key(contract_id, tx)

            if method_name == 'initialize':
                if indexes.nc_creation is not None:
                    indexes.nc_creation.manually_add_tx(tx)

                if indexes.blueprint_history is not None:
                    nc_storage = runner.get_storage(contract_id)
                    blueprint_id = nc_storage.get_blueprint_id()
                    indexes.blueprint_history.add_single_key(blueprint_id, tx)

        # Update metadata.
        assert tx_meta.nc_calls is None
        tx_meta.nc_calls = nc_calls
        self.context.save(tx)

    def nc_remove_indexes(self, block: Block, tx: Transaction) -> None:
        assert tx.storage is not None
        assert tx.storage.indexes is not None
        indexes = tx.storage.indexes
        tx_meta = tx.get_metadata()
        nc_calls_set = set(tx_meta.nc_calls or [])

        block_root_id = block.get_metadata().nc_block_root_id
        assert block_root_id is not None
        block_trie = self.context.consensus.nc_storage_factory.get_trie(block_root_id)
        ...
        for contract_id, method_name in nc_calls_set:
            if indexes.nc_history is not None:
                indexes.nc_history.remove_single_key(contract_id, tx)

            if method_name == 'initialize':
                if indexes.nc_creation is not None:
                    indexes.nc_creation.del_tx(tx)

                if indexes.blueprint_history is not None:
                    nc_root_id = block_trie.get(contract_id)
                    nc_storage = self.context.consensus.nc_storage_factory(contract_id, nc_root_id)
                    blueprint_id = nc_storage.get_blueprint_id()
                    indexes.blueprint_history.remove_single_key(blueprint_id, tx)
        ...
        for tx in bfs.run(block, skip_root=True):
            ...
            if tx.is_nano_contract():
                if meta.nc_execution is NCExecutionState.SUCCESS:
                    assert isinstance(tx, Transaction)
                    self.nc_remove_indexes(block, tx)
