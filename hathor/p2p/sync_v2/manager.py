...
from hathor.p2p.sync_v2.request_response_command import MultipleRequestResponseCommands, RequestResponseCommand
...
class NodeBlockSync(SyncAgent):
    ...
    def __init__(self, protocol: 'HathorProtocol', reactor: Optional[Reactor] = None) -> None:
        ...
        self._get_tx_cmds = MultipleRequestResponseCommands[VertexId, BaseTransaction](
            message_sender=self.send_message,
            request_message=ProtocolMessages.GET_DATA,
        )

        self._get_peer_block_hashes_cmd = RequestResponseCommand[list[tuple[int, bytes]]](
            message_sender=self.send_message,
            request_message=ProtocolMessages.GET_PEER_BLOCK_HASHES,
        )

        self._get_best_block_cmd = RequestResponseCommand[dict[str, Any]](
            message_sender=self.send_message,
            request_message=ProtocolMessages.GET_BEST_BLOCK,
        )

        self._get_tips_cmd = RequestResponseCommand[list[bytes]](
            message_sender=self.send_message,
