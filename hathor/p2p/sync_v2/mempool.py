...
class SyncMempoolManager:
    ...
    def _unsafe_run(self) -> Generator[Deferred, Any, bool]:
        ...
        if not self.missing_tips:
            ...
            self.missing_tips.update(h for h in tx_hashes if not self.sync_agent.p2p_storage.transaction_exists(h))
    ...
    def _next_missing_dep(self, tx: BaseTransaction) -> Optional[bytes]:
        ...
        for txin in tx.inputs:
            if not self.sync_agent.p2p_storage.transaction_exists(txin.tx_id):
                return txin.tx_id
        for parent in tx.parents:
            if not self.sync_agent.p2p_storage.transaction_exists(parent):
                return parent
        return None
