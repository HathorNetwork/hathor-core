...
class DNSPeerDiscovery(PeerDiscovery):
    ...
    async def dns_seed_lookup(self, host: str) -> set[PeerAddress]:
        ...
        if self.test_mode:
            ...
            return {PeerAddress.parse('tcp://127.0.0.1:40403')}
        ...
        deferreds.append(cast(Deferred[Iterator[PeerAddress]], d1))  # mypy doesn't know how addCallback affects d1

        d2 = self.do_lookup_address(host)
        d2.addCallback(self.dns_seed_lookup_address)
        d2.addErrback(self.errback)
        deferreds.append(cast(Deferred[Iterator[PeerAddress]], d2))  # mypy doesn't know how addCallback affects d2

        results: list[Iterator[PeerAddress]] = await gatherResults(deferreds)
    ...
    def dns_seed_lookup_text(self, results: LookupResult) -> Iterator[PeerAddress]:
        ...
        for record in answers:
            ...
            for txt in record.payload.data:
                ...
                try:
                    entrypoint = PeerAddress.parse(raw_entrypoint)
                except ValueError:
    ...
    def dns_seed_lookup_address(self, results: LookupResult) -> Iterator[PeerAddress]:
        ...
        for record in answers:
            ...
            entrypoint = PeerAddress(Protocol.TCP, host, self.default_port)
