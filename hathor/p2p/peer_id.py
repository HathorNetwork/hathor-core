...
class PeerId:
    ...
    def increment_retry_attempt(self, now: int) -> None:
        ...
        self.retry_interval = self.retry_interval * constants.PEER_CONNECTION_RETRY_INTERVAL_MULTIPLIER
        if self.retry_interval > constants.PEER_CONNECTION_RETRY_MAX_RETRY_INTERVAL:
            self.retry_interval = constants.PEER_CONNECTION_RETRY_MAX_RETRY_INTERVAL
    ...
    def get_certificate(self) -> x509.Certificate:
        if not self.certificate:
            assert self.private_key is not None
            certificate = generate_certificate(self.private_key, constants.CA_FILEPATH, constants.CA_KEY_FILEPATH)
    ...
    def _get_certificate_options(self) -> CertificateOptions:
        ...
        with open(constants.CA_FILEPATH, 'rb') as f:
