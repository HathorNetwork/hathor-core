...
class VertexHelper:
    ...
    def decode_nc_args(self, tx: 'Transaction') -> Any:
        """Decode nano contract arguments if the transaction was successfully executed.

        Returns a list of JSON-serialized argument strings, or None if decoding is not applicable.
        Each argument that fails to serialize to JSON will be replaced with "error".
        """
        from hathor.nanocontracts.method import Method
        from hathor.nanocontracts.types import BlueprintId
        from hathor.transaction.nc_execution_state import NCExecutionState

        # Only decode if execution was successful
        meta = tx.get_metadata()
        if meta.nc_execution != NCExecutionState.SUCCESS:
            return None

        # Get blueprint_id from the first nc_calls record
        if not meta.nc_calls or len(meta.nc_calls) == 0:
            return None

        blueprint_id = BlueprintId(meta.nc_calls[0].blueprint_id)

        # As tx has already been executed, so the blueprint and method must exist and
        # the deserialization must work smoothly.
        nano_header = tx.get_nano_header()
        blueprint_class = self.tx_storage.get_blueprint_class(blueprint_id)
        method_callable = getattr(blueprint_class, nano_header.nc_method)
