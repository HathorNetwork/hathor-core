...
from hathor.verification.verification_dispatcher import VerificationDispatcher
...
class VertexHandler:
    ...
    def __init__(
        ...
        *,
        ...
        self._pubsub = pubsub
        self._wallet = wallet
        self._verification_dispatcher = VerificationDispatcher(  # TODO: Move to builder
    ...
    async def on_new_vertex_async(
        ...
        *,
        ...
        if self._verification_dispatcher.is_processing(vertex.hash):
            # TODO: If False,
    ...
    async def _validate_vertex_async(
        ...
        *,
        ...
        if not metadata or not metadata.validation.is_fully_connected():
            try:
                await self._verification_dispatcher.validate_full(vertex, reject_locked_reward=reject_locked_reward)
            except HathorError as e:
                if not fails_silently:
