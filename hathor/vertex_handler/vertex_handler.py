...
class VertexHandler:
    ...
    async def _validate_vertex_async(
        ...
        *,
        ...
        if not metadata or not metadata.validation.is_fully_connected():
            try:
                await self._verifier.validate_full(vertex, reject_locked_reward=reject_locked_reward)
            except HathorError as e:
                if not fails_silently:
    ...
    def _log_new_object(self, tx: Vertex, message_fmt: str, *, quiet: bool) -> None:
        ...
        if tx.is_block:
            ...
            if isinstance(tx, Block):
                ...
                kwargs['feature_states'] = {
                    feature.value: state.value
                    for feature, state in tx.static_metadata.feature_states.items()
                }
        else:
    ...
    def _log_active_features(self, vertex: Vertex) -> None:
        ...
        for feature in self._settings.FEATURE_ACTIVATION.features:
            if vertex.static_metadata.is_feature_active(feature):
                self._log.info(
