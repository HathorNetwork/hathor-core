...
class SwapDemo(Blueprint):
    ...
    def initialize(self,
                   ...
                   token_b: TokenUid,
                   multiplier_a: int,
                   multiplier_b: int) -> None:
        """Initialize the contract."""
        if set(ctx.actions.keys()) != {token_a, token_b}:
            raise InvalidTokens

        if set(x.type for x in ctx.actions.values()) != {NCActionType.DEPOSIT}:
            raise InvalidActions('only deposits allowed')
    ...
    def swap(self, ctx: Context) -> None:
        ...
        if set(x.type for x in ctx.actions.values()) != {NCActionType.WITHDRAWAL, NCActionType.DEPOSIT}:
