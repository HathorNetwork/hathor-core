...
class Node:
    ...
    Each node can carry a value or not. If a node does not carry a value, its key has never been directly
    added to the trie but it was created because some keys have the same prefix.
    ...
    def copy(self, value: Optional[bytes] = None, children: Optional[DictChildren] = None) -> 'Node':
        ...
        return Node(key=self.key, length=self.length, value=value, children=children)
...
class PatriciaTrie:
    ...
    def iter_dfs(self, *, node: Optional[Node] = None, depth: int = 0) -> Iterable[IterDFSNode]:
    ...
    def print_dfs(self, node: Optional[Node] = None, *, depth: int = 0) -> None:
        ...
        print(f'{prefix}value: {node.value!r}')
    ...
    def _update(self, key: bytes, value: bytes) -> None:
        ...
        if parent.key == key:
            ...
            new_nodes.append((parent_match, parent.copy(value=value)))
        ...
        for k, _v in parent.children.items():
            ...
            if split.key == key:
                # If they are equal, the split node will store the value and we are done.
                split.value = value
        ...
        # Finally, create the new node that will store the value.
    ...
    def _get(self, key: bytes, *, root_id: Optional[NodeId] = None) -> bytes:
        """Internal method to get the value of a key."""
        if key == b'':
    ...
    def update(self, key: bytes, value: bytes) -> None:
        """Update the value of a key. This method might change the root of the trie."""
        real_key = self._encode_key(key)
        return self._update(real_key, value)
