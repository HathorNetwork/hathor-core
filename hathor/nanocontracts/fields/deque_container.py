...
from collections.abc import Container as ContainerAbc, Iterable, Iterator, Sequence
...
class DequeContainer(Container[T]):
    ...
    __slots__ = ('__storage', '__prefix', '__value_node', '__metadata_key')
    ...
    def __init__(self, storage: NCContractStorage, prefix: bytes, value_node: ContainerNode[T]) -> None:
        ...
        self.__metadata_key = KEY_SEPARATOR.join([self.__prefix, _METADATA_KEY])
    ...
    def __from_prefix_and_type__(
        ...
        *,
        ...
        item_node = ContainerNode.from_type(storage, item_type, type_map=type_map)
        return cls(storage, prefix, item_node)

    @override
    def __init_storage__(self) -> None:
        self.__storage.put_obj(self.__metadata_key, _METADATA_NC_TYPE, _DequeMetadata())
    ...
    def __to_db_key(self, index: SupportsIndex) -> bytes:
        return KEY_SEPARATOR.join([self.__prefix, _INDEX_NC_TYPE.to_bytes(index.__index__())])

    def __get_metadata(self) -> _DequeMetadata:
        return self.__storage.get_obj(self.__metadata_key, _METADATA_NC_TYPE)
