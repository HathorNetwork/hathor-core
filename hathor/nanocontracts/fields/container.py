...
from hathor.nanocontracts.blueprint_env import Cache
...
class ContainerNodeFactory(Generic[T]):
    ...
    def __init__(self, type_: type[T], type_map: Field.TypeMap) -> None:
        ...
        if origin_type in type_map.container_map:
            ...
            container_class.__check_type__(type_)  # type: ignore[arg-type]
            self.is_container = True
        else:
            # XXX: doing this just to check the type
            _ = NCType.from_type(type_, type_map=type_map.to_nc_type_map())
...
class ContainerNode(ABC, Generic[T]):
    ...
    def __init__(self, storage: NCContractStorage, cache: Cache):
...
class ContainerLeaf(ContainerNode[T]):
    ...
    def __init__(self, storage: NCContractStorage, nc_type: NCType[T], cache: Cache = None) -> None:
        super().__init__(storage, cache)
