...
class SetStorageContainer(StorageContainer, MutableSet[T]):
    ...
    def __check_name_and_type__(cls, name: str, type_: type[set[T]]) -> None:
        ...
        origin_type: type[set[T]] = get_origin(type_) or type_
    ...
    @override
    def __contains__(self, elem: T, /) -> bool:  # type: ignore[override]
    ...
    #     key = self.__to_db_key(elem)
    #     if not self.__storage.contains(key):
    #         raise KeyError
    #     self.__storage.del_obj(key)
    #     self.__decrease_length()
