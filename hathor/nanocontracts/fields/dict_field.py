...
class DictStorageContainer(StorageContainer, MutableMapping[K, V]):
    ...
    def __check_name_and_type__(cls, name: str, type_: type[Mapping[K, V]]) -> None:
        ...
        origin_type: type[Mapping[K, V]] = get_origin(type_) or type_
    ...
    # Methods provided by MutableMapping
    ...
    # `override` is ignored because the signature expects `key: object`, but it's OK to force `key: K` in our case
    @override
    ...
    # XXX: `assignment` is ignored because mypy can't reason about _NOT_PROVIDED
    # XXX: `misc` is ignored because mypy thinks this function does not accept all arguments of the second get overload
    @override  # type: ignore[misc]
    def get(self, key: K, /, default: V | _T = _NOT_PROVIDED) -> V | _T:  # type: ignore[assignment]
