...
class MappingProxyNCType(_MapNCType):
    """ Represents builtin `types.MappingProxyType` values.
    """

    __slots__ = ('_is_hashable', '_key', '_value')

    @override
    def __init__(self, key: NCType[H], value: NCType[T]) -> None:
        super().__init__(key, value)
        # key is already enforced to be hashable, but a mappingproxy itself is only hashable if its value is hashable
        self._is_hashable = value.is_hashable()

    @override
    def _build(self, items: Iterable[tuple[H, T]]) -> mappingproxy[H, T]:
        # XXX: we need to construct a fresh dict first and then make the mappingproxy
        return mappingproxy(dict(items))
