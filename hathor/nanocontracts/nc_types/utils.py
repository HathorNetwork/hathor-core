...
def is_origin_hashable(type_: type) -> bool:
    ...
    """
    return all(is_subclass(origin_class, Hashable) for origin_class in get_origin_classes(type_))
...
def get_aliased_type(type_: type | UnionType, alias_map: TypeAliasMap, *, _verbose: bool = True) -> type:
    ...
    For example, `set` is mapped to `frozenset`, and `dict` to `mappingproxy` in the default alias map:
    ...
    tuple[str, frozenset[frozenset[mappingproxy[int, frozenset[str]]]], frozenset[int], bool]
    """
...
def get_usable_origin_type(
    ...
    *,
    ...
    For example, if we have a type `dict[int, int]` it cannot be used to index the default types map, its origin
    however, is `dict`, which also isn't in the default map, but after applying the alias it becomes a `mappingproxy`,
