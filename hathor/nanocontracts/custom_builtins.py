...
def _generate_restricted_import_function(
    runner: Runner | None,
    ...
    If a Runner is provided, it will be injected in lazy imports, otherwise an invalid import will be generated.
    """
    ...
    def __import__(
        ...
        for import_what in fromlist:
            ...
            import_value = allowed_fromlist[import_what]
            from hathor.nanocontracts.lazy_import import LazyImport
            if isinstance(import_value, LazyImport):
                # Lazy imports are created here, at the time they're imported, using the provided Runner.
                import_value = import_value.create_with_runner(runner)

            setattr(fake_module, import_what, import_value)
...
def get_exec_builtins(*, runner: Runner | None) -> dict[str, Any]:
    """
    Dynamically generate a list of allowed builtins during execution of a contract,
    with an optional Runner that is used to instantiate lazy imports.
    """
    return {
        ...
        '__import__': _generate_restricted_import_function(runner, ALLOWED_IMPORTS),

        # XXX: also required to declare classes
        # XXX: this would be '__main__' for a module that is loaded as the main entrypoint,
        # and the module name otherwise, since the blueprint code is adhoc,
        # we could as well expose something else, like '__blueprint__' constant
