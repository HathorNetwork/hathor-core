...
class Runner:
    ...
    def get_blueprint_id(self, nanocontract_id: ContractId) -> Type[Blueprint]:
    ...
    def _unsafe_call_another_contract_public_method(
        ...
        **kwargs: Any
        ...
        Used by call_another_contract_public_method() and create_another_nano_contract()."""
        assert self._call_info is not None
    ...
    def _create_contract(self, nanocontract_id: ContractId, blueprint_id: BlueprintId) -> None:
        nc_storage = self.storage_factory(nanocontract_id, None)
        nc_storage.set_blueprint_id(blueprint_id)
        # nc_storage.set_created_by(vertex_id)
        # TODO Remove this commit?
        nc_storage.commit()
        self.block_trie.update(nanocontract_id, nc_storage.get_root_id())
    ...
    def create_contract(self,
        ...
        self._create_contract(nanocontract_id, blueprint_id)
        try:
    ...
    def create_another_nanocontract(self,
                                    blueprint_id: BlueprintId,
                                    salt: bytes,
                                    actions: list[NCAction],
                                    *args: Any,
        ...
        self._create_contract(child_id, blueprint_id)
    ...
    def change_blueprint(self, blueprint_id: BlueprintId):
        last_call_record = self._call_info.stack[-1]
        if last_call_record.type is CallType.VIEW:
            raise NCInvalidPublicMethodCallFromView('cannot create a contract from a view method')

        nc_storage = self.get_storage(last_call_record.nanocontract_id)
        nc_storage.set_blueprint_id(blueprint_id)
