...
class Runner:
    ...
    def syscall_mint_tokens(self, token_uid: TokenUid, amount: int,
    ...
    def _get_token(self, token_uid: TokenUid) -> TokenDescription:
        ...
        # TODO-RAUL: do we need to check the tx storage?
        # try:
        ...
        #     )
        # except Exception:
        #     raise NCInvalidSyscall(f'contract {call_record.contract_id.hex()} could not find {token_uid.hex()} token')
        raise NCInvalidSyscall(f'contract {call_record.contract_id.hex()} could not find {token_uid.hex()} token')
