...
from typing import Any, Callable, Concatenate, ParamSpec, Type, TypeVar
...
class Runner:
    ...
    def _create_changes_tracker(self, contract_id: ContractId) -> NCChangesTracker:
        ...
        change_tracker = NCChangesTracker(contract_id, storage)
    ...
    def get_blueprint_class(self, contract_id: ContractId) -> Type[Blueprint]:
    ...
    def _unsafe_call_public_method(
        ...
        ret = self._execute_public_method_call(contract_id, method_name, ctx, nc_args)
    ...
    def syscall_call_another_contract_public_method(
        ...
        return self._unsafe_call_another_contract_public_method(contract_id, method_name, actions, args, kwargs)
    ...
    def _unsafe_call_another_contract_public_method(
        ...
        if last_call_record.contract_id == contract_id:
        ...
        nc_args = NCParsedArgs(args, kwargs)
        return self._execute_public_method_call(contract_id, method_name, ctx, nc_args)
    ...
    def _execute_public_method_call(
        ...
        """
        from hathor.nanocontracts.types import NC_FALLBACK_METHOD, NC_INITIALIZE_METHOD
        ...
        blueprint_id = self.get_blueprint_id(contract_id)
        blueprint = self._create_blueprint_instance(contract_id, changes_tracker)
        ...
        if method is None:
            ...
            if not is_nc_public_method(method):
                raise NCError(f'method `{method_name}` is not a public method')
            match nc_args:
    ...
    def _create_blueprint_instance(self, contract_id: ContractId, changes_tracker: NCChangesTracker) -> Blueprint:
        """Create a new blueprint instance."""
        assert self._call_info is not None
        env = BlueprintEnvironment(self, self._call_info.nc_logger, changes_tracker)
        blueprint_class = self.get_blueprint_class(contract_id)
