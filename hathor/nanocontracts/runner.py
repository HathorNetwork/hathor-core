...
class Runner:
    ...
    def get_storage(self, nanocontract_id: ContractId) -> NCStorage:
        ...
        if storage is None:
            try:
                ...
                # TODO Only allow this to the first caller and only when it is creating a contract.
                nc_root_id = None
            storage = self.storage_factory(nanocontract_id, nc_root_id)
            self._storages[nanocontract_id] = storage
        return storage

    def get_changes_tracker(self, nanocontract_id: ContractId) -> NCChangesTracker:
    ...
    def _get_single_runner(self, nanocontract_id: ContractId) -> _SingleCallRunner:
        ...
        change_tracker = self.get_changes_tracker(nanocontract_id)
    ...
    def call_another_contract_public_method(self,
        ...
        changes_tracker = self.get_changes_tracker(last_nanocontract_id)
