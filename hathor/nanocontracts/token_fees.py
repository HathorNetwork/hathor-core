    ...
    *,
    ...
    match token_version:
        ...
        case TokenVersion.DEPOSIT:
            _validate_deposit_based_token(fee_payment_token)
            return -get_deposit_token_deposit_amount(settings, amount)
        case TokenVersion.FEE:
            _validate_fee_based_token(fee_payment_token)
            return -_calculate_fee_token_fee(settings, fee_payment_token)
...
def _validate_fee_based_token(fee_payment_token: TokenDescription) -> None:
...
def _validate_deposit_based_token(fee_payment_token: TokenDescription) -> None:
...
def _calculate_fee_token_fee(settings: HathorSettings, fee_payment_token: TokenDescription) -> int:
    """Calculate the fee for an handling a fee-based token"""
    from hathor import HATHOR_TOKEN_UID
    if fee_payment_token.token_id == HATHOR_TOKEN_UID:
