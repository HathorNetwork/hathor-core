...
class Builder:
    ...
    def _get_or_create_sandbox_api_config_loader(self) -> SandboxAPIConfigLoader | None:
        ...
        The loader is only created if nano contracts are enabled and a config file is specified.
        Returns None if nano contracts are disabled or no config file is set.
        """
        if self._sandbox_api_config_loader is None:
            settings = self._get_or_create_settings()
            if settings.ENABLE_NANO_CONTRACTS and settings.NC_SANDBOX_API_CONFIG_FILE:
                self._sandbox_api_config_loader = SandboxAPIConfigLoader(
                    settings.NC_SANDBOX_API_CONFIG_FILE
                )
