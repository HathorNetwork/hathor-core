...
class TransactionMetadata:
    ...
    def to_storage_json(self) -> dict[str, Any]:
        ...
        data['nc_calls'] = [(x.contract_id.hex(), x.method_name) for x in self.nc_calls] if self.nc_calls else None
    ...
    def create_from_json(cls, data: dict[str, Any]) -> 'TransactionMetadata':
        ...
        if nc_calls_raw is not None:
            meta.nc_calls = [MetaNCCallRecord(bytes.fromhex(x[0]), x[1]) for x in nc_calls_raw]
        else:
            meta.nc_calls = None
