...
class ShieldedOutputMode(IntEnum):
    """Privacy level for a shielded output."""
    AMOUNT_ONLY = 0       # Amount hidden, token ID visible (no surjection proof)
    FULLY_SHIELDED = 1    # Both amount and token ID hidden (surjection proof required)
...
class AmountShieldedOutput:
    ...
    @staticmethod
    def mode() -> ShieldedOutputMode:
        return ShieldedOutputMode.AMOUNT_ONLY
...
class FullShieldedOutput:
    ...
    @staticmethod
    def mode() -> ShieldedOutputMode:
        return ShieldedOutputMode.FULLY_SHIELDED
...
def deserialize_shielded_output(buf: bytes | memoryview) -> tuple[ShieldedOutput, bytes]:
    ...
    offset += 1
    mode = ShieldedOutputMode(mode_byte)
    ...
    if mode == ShieldedOutputMode.AMOUNT_ONLY:
    ...
    elif mode == ShieldedOutputMode.FULLY_SHIELDED:
...
def get_sighash_bytes(output: ShieldedOutput) -> bytes:
    ...
    # Include ephemeral pubkey in sighash to prevent MITM tampering
    if output.ephemeral_pubkey:
        parts.append(output.ephemeral_pubkey)
