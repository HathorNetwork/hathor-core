...
class Migration(BaseMigration):
    ...
    def get_db_name(self) -> str:
        return 'add_closest_ancestor_block_metadata'

    def run(self, storage: 'TransactionStorage') -> None:
        """
        This migration calculates closest ancestor block and feature states for txs and saves them as static metadata.
        """
        ...
        for vertex in progress(topological_iterator, log=log, total=None):
            if not isinstance(vertex, Transaction):
                continue
            vertex._static_metadata = TransactionStaticMetadata.create_from_storage(vertex, settings, storage)
