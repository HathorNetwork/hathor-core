...
NANO_HEADER_ID = b'\xa5'
assert len(NANO_HEADER_ID) == 1
...
class NanoHeader(VertexBaseHeader):
    ...
    def deserialize(
        ...
        *,
        ...
        assert header_id == NANO_HEADER_ID, f'{header_id!r} {NANO_HEADER_ID!r}'
    ...
    def serialize(self, *, skip_header_id: bool = False, skip_signature: bool = False) -> bytes:
        """Serialize the method call as part of a transaction serialization."""
        encoded_method = self.nc_method.encode('ascii')

        ret = []
        if not skip_header_id:
            ret.append(NANO_HEADER_ID)
    ...
    def get_sighash_bytes(self) -> bytes:
        return self.serialize(skip_header_id=True, skip_signature=True)
