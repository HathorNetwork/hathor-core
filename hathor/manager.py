...
class HathorManager:
    ...
    def on_new_tx(self, tx: BaseTransaction, *, conn: Optional[HathorProtocol] = None,
        ...
        try:
            ...
            self.log.warn('on_new_tx(): step validations failed', tx=tx.hash_hex)
            return False
...
class ParentTxs(NamedTuple):
    ...
    def get_random_parents(self, rng: Rng) -> Tuple[bytes, bytes]:
        ...
        fill = random_choices(rng, self.can_include, 2 - len(self.must_include))
        fill.sort(key=lambda tx_hash: self.can_include.index(tx_hash))
