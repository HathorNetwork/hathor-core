...
class IndexesManager(ABC):
    ...
    def update_critical_indexes(self, tx: BaseTransaction, *, is_new: bool = False) -> None:
        ...
        `is_new` marks the vertex being connected by the current consensus update (it cannot have
        children/spenders yet — see MempoolTipsIndex.update). Only used by the S5-optimized path.
        """
