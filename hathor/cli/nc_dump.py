...
def get_sorted_key_values_from_trie(trie: PatriciaTrie) -> OrderedDict[bytes, bytes]:
    key_values = {}
    ...
    if key_values:
        ...
        return OrderedDict(sorted(key_values.items(), key=(lambda kv: (-kv[0][0], kv[0][1:]))))
    else:
        return OrderedDict([])
    ...
    def run(self) -> None:
        ...
        for block_height, block_hash, block_root_id in collected_blocks:
            ...
            for contract_id, contract_root_id in contract_root_ids.items():
                ...
                if contract_key_values := get_sorted_key_values_from_trie(block_storage._get_trie(contract_root_id)):
